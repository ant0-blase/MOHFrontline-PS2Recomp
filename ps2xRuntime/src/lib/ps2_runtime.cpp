...
// [MOH] VIF1 i-bit INTC-5 pump plumbing + host audio-bridge pump (defined in
// ps2_memory.cpp / game_overrides.cpp).
extern std::function<void()> g_vif1InterruptCallback;
void mohPumpHostAudioBridge();
...
// [MOH diag] external accessor for the dispatch-PC chain (GS rasterizer, overrides).
...
static void UploadFrame(Texture2D &tex, PS2Runtime *rt, uint32_t &outWidth, uint32_t &outHeight)
{
    ...
    // [MOH] Drain the EA audio bridge once per vsync tick, not per host
    // present: uncapped presents would starve the decoder threads on the
    // bridge mutex and stall the movie pipeline.
    {
        static uint64_t s_lastAudioPumpTick = std::numeric_limits<uint64_t>::max();
        if (currentTick != s_lastAudioPumpTick)
        {
            s_lastAudioPumpTick = currentTick;
            mohPumpHostAudioBridge();
        }
    }
...
bool PS2Runtime::syncCoreSubsystems()
{
    ...
    // [MOH] VIF1 i-bit INTC-5 upload-pump dispatch (frontend textures).
    g_vif1InterruptCallback = [this]()
    {
        static std::atomic<uint32_t> s_dispatchCount{0u};
        const uint32_t n = s_dispatchCount.fetch_add(1u, std::memory_order_relaxed);
        if (n < 24u || (n & 0x1FFu) == 0u)
        {
            std::cerr << "[MOH:vif1-intc] dispatch n=" << n << std::endl;
        }
        ps2_syscalls::dispatchVif1IntcHandlers(m_memory.getRDRAM(), this);
    };
...
// [MOH] Dense recompiled-function table storage. PR137's recompiler normally
// emits these; MOH's recomp output registers at runtime instead, so the
// storage lives here and registerAllFunctions() fills the slots.
// core 0x00104008..0x00237d14, launcher 0x01c00008..~0x01c1b000; the
// inter-range gap is sparse BSS (demand-zero).
extern const uint32_t g_ps2RecompiledFunctionTableBase = 0x00104008u;
extern const uint32_t g_ps2RecompiledFunctionTableEnd = 0x01c1b000u;
extern const uint32_t g_ps2RecompiledFunctionTableSlotCount =
    (0x01c1b000u - 0x00104008u) / 4u;
PS2Runtime::RecompiledFunction g_ps2RecompiledFunctionTable[
    (0x01c1b000u - 0x00104008u) / 4u] = {};

// [MOH] Allocator no-preempt depth (game_overrides bumps it; inert under N=1).
std::atomic<uint32_t> g_mohGuestNoPreemptDepth{0u};
...
// [MOH compat] A2-guard hook; cooperative yield under the fiber executor.
void PS2Runtime::yieldGuestExecutionAfterWake()
{
    (void)ps2sched::yield_point();
}
