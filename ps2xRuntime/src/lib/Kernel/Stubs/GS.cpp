...
namespace ps2_stubs
{
    namespace
    {
        ...
        void logMohPostC0GsCallbackStepTraceV1(
            const char *phase,
            uint64_t tick,
            uint32_t callback,
            uint32_t stepIndex,
        ...
        {
            if (!mohIsPostC0FrameWaitSeenForInterruptTraceV2())
            {
                return;
            }
            ...
            std::cerr
                << "[MOH:post-c0-gs-callback-step-trace-v1]"
                << " idx=" << std::dec << index
                << " phase=" << phase
                << " tick=0x" << std::hex << tick
                << " callback=0x" << callback
                << " stepIndex=0x" << stepIndex
                << " stepPc=0x" << stepPc
                << " nextPc=0x" << ctx.pc
        ...
        bool readMohStartBmp(std::vector<uint8_t> &out)
        {
            ...
            const std::array<std::filesystem::path, 4> relatives = {
                std::filesystem::path("DATA") / "START.BMP",
                std::filesystem::path("data") / "start.bmp",
                std::filesystem::path("Data") / "Start.bmp",
                std::filesystem::path("START.BMP")
            };
        ...
        bool patchMohStartBmpUpload(uint8_t *rdram,
                                    uint32_t dataAddr,
                                    uint32_t width,
                                    uint32_t height,
                                    uint32_t qwcRemaining)
        {
            constexpr uint32_t kStartBmpBytes = 512u * 256u * 3u;
            if (!rdram ||
                width != 512u ||
                height != 256u ||
                qwcRemaining * 16u < kStartBmpBytes)
            {
                return false;
            }

            uint8_t *dst = getMemPtr(rdram, dataAddr);
            if (!dst)
            {
                return false;
            }
            ...
            std::vector<uint8_t> rgb;
            if (!readMohStartBmp(bmp) || !decodeBmp24ToRgbTriples(bmp, width, height, rgb))
            {
                return false;
            }
    ...
    {
        const uint32_t stateAddr = getRegU32(ctx, 4);
        uint32_t dbp = getRegU32(ctx, 5) & 0xFFFFu;
        ...
        if (dbp == 0u &&
            ...
            dsax == 0u &&
            dsay == 0u &&
            patchMohStartBmpUpload(rdram, dataAddr, width, height, qwcRemaining))
        {
            dbp = 0x3800u;
            static uint32_t s_mohStartBmpUploadLogs = 0u;
            if (s_mohStartBmpUploadLogs < 4u)
            {
        ...
        // Open a 4-register A+D GIF tag and emit the GS load-image setup.
        {
            ...
            // [A/B test] original pre-seeded 4-register A+D tag; the "open tag"
            // variant is suspected of stalling the INTRO movie stream.
    ...
    void dispatchGsSyncVCallback(uint8_t *rdram, PS2Runtime *runtime, uint64_t tick)
    {
        ...
        try
        {
            ...
            while (callbackCtx.pc != 0u && !runtime->isStopRequested() && steps < 1024u)
            {
                ...
                ++steps;
                const uint32_t stepPc = callbackCtx.pc;
                logMohPostC0GsCallbackStepTraceV1(
                    "step-enter",
                    callbackTick,
                    callback,
                    steps,
    ...
    void sceGsExecLoadImage(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        constexpr uint32_t kMaxGifImageQwc = 0x7FFFu;
        const uint32_t setupQwc = 5u;
        const uint32_t imageQwc = (totalImageBytes + 15u) / 16u;
        const uint32_t imageChunkCount =
            (imageQwc == 0u) ? 0u : ((imageQwc + kMaxGifImageQwc - 1u) / kMaxGifImageQwc);
        const uint32_t totalQwc = setupQwc + imageQwc + imageChunkCount;
        ...
        if (dbp == 0u &&
            ...
            dsax == 0u &&
            dsay == 0u &&
            patchMohStartBmpUpload(rdram, srcAddr, img.width, img.height, imageQwc))
        {
            dbp = 0x3800u;
            static uint32_t s_mohStartBmpExecUploadLogs = 0u;
            if (s_mohStartBmpExecUploadLogs < 4u)
            {
                std::cerr << "[MOH:start-bmp-upload]"
                          << " mode=exec"
                          << " data=0x" << std::hex << srcAddr
                          << " dbp=0x3800"
                          << " fbw=0x" << fbw
                          << " qwc=0x" << imageQwc
                          << std::dec
        ...
        uint32_t qwordOffset = setupQwc;
        uint32_t copiedBytes = 0u;
        uint32_t qwcRemaining = imageQwc;
        while (qwcRemaining != 0u)
        {
            const uint32_t chunkQwc = std::min<uint32_t>(qwcRemaining, kMaxGifImageQwc);
            const bool isLastChunk = (chunkQwc == qwcRemaining);
            uint64_t *tag = reinterpret_cast<uint64_t *>(pkt + (qwordOffset * 16u));
            tag[0] = (static_cast<uint64_t>(2u) << 58) |
                     static_cast<uint64_t>(chunkQwc) |
                     (isLastChunk ? (1ULL << 15) : 0ULL);
            tag[1] = 0ULL;
            ++qwordOffset;

            const uint32_t chunkCapacityBytes = chunkQwc * 16u;
            const uint32_t bytesLeft = totalImageBytes - copiedBytes;
            const uint32_t chunkBytes = std::min<uint32_t>(bytesLeft, chunkCapacityBytes);
            std::memcpy(pkt + (qwordOffset * 16u), src + copiedBytes, chunkBytes);
            copiedBytes += chunkBytes;
            qwordOffset += chunkQwc;
            qwcRemaining -= chunkQwc;
        }
