...
namespace
{
    ...
    {
        const std::filesystem::path configured = PS2Runtime::getIoPaths().cdImage;
        if (!configured.empty())
        {
            return configured;
        }

        // MOH Frontline local fallback: allow raw sceCdRead(LBN) from the original ISO.
        const std::filesystem::path candidates[] = {
            "iso/mohfrontline.iso",
            "ISO/mohfrontline.iso",
            "iso/MOHFRONTLINE.ISO",
            "ISO/MOHFRONTLINE.ISO",
        };
        ...
        for (const std::filesystem::path &candidate : candidates)
        {
            ...
            if (std::filesystem::exists(candidate, ec) && !ec)
            {
                ...
                return absEc ? candidate : abs.lexically_normal();
            }
        }
