...
namespace ps2_stubs
{
    namespace
    {
        void traceMpegStubCall(const char *name, const R5900Context *ctx)
        {
            static std::atomic<uint32_t> s_count{0u};
            const uint32_t index = s_count.fetch_add(1u, std::memory_order_relaxed);
            if (index >= 192u || !ctx)
            {
                return;
            }

            std::cerr << "[MPEG:call] #" << index
                      << " name=" << name
        ...
        struct MpegDecodedFrame
        {
            int width = 0;
            int height = 0;
            uint32_t frameRateNumerator = 0u;
            uint32_t frameRateDenominator = 0u;
            uint32_t repeatPict = 0u;
        ...
        public:
            ...
            bool feed(const uint8_t *data, size_t size, std::deque<MpegDecodedFrame> &frames)
            {
                ...
                updateFrameRateFromSequenceHeader(data, size);
            ...
            void reset()
            {
                ...
                m_initialized = false;
                m_drained = false;
                m_frameRateNumerator = 0u;
                m_frameRateDenominator = 0u;
            }

        private:
            void updateFrameRateFromSequenceHeader(const uint8_t *data, size_t size)
            {
                if (!data || size < 8u || (m_frameRateNumerator != 0u && m_frameRateDenominator != 0u))
                {
                    return;
                }

                for (size_t offset = 0u; offset + 8u <= size; ++offset)
                {
                    if (data[offset + 0u] != 0x00u ||
                        data[offset + 1u] != 0x00u ||
                        data[offset + 2u] != 0x01u ||
                        data[offset + 3u] != 0xB3u)
                    {
                        continue;
                    }

                    // MPEG-1/2 sequence_header frame_rate_code.  The audited
                    // MOH streams use code 4 (30000/1001); their sequence
                    // extension rate numerator/denominator are both zero.
                    switch (data[offset + 7u] & 0x0Fu)
                    {
                    case 1u: m_frameRateNumerator = 24000u; m_frameRateDenominator = 1001u; break;
                    case 2u: m_frameRateNumerator = 24u; m_frameRateDenominator = 1u; break;
                    case 3u: m_frameRateNumerator = 25u; m_frameRateDenominator = 1u; break;
                    case 4u: m_frameRateNumerator = 30000u; m_frameRateDenominator = 1001u; break;
                    case 5u: m_frameRateNumerator = 30u; m_frameRateDenominator = 1u; break;
                    case 6u: m_frameRateNumerator = 50u; m_frameRateDenominator = 1u; break;
                    case 7u: m_frameRateNumerator = 60000u; m_frameRateDenominator = 1001u; break;
                    case 8u: m_frameRateNumerator = 60u; m_frameRateDenominator = 1u; break;
                    default: break;
                    }
                    return;
                }
            ...
            bool convertFrame(std::deque<MpegDecodedFrame> &frames)
            {
                ...
                decoded.frameRateNumerator = m_frameRateNumerator;
                decoded.frameRateDenominator = m_frameRateDenominator;
                decoded.repeatPict = static_cast<uint32_t>(std::max(m_frame->repeat_pict, 0));
            ...
            uint32_t m_frameRateNumerator = 0u;
            uint32_t m_frameRateDenominator = 0u;
        };
        ...
        struct MpegPlaybackState
        {
            ...
            bool presentationPacingStarted = false;
            uint64_t nextPresentationVsync = 0u;
            uint64_t presentationPhase = 0u;
            uint64_t firstPresentationVsync = 0u;
            uint64_t lastPresentationVsync = 0u;
            bool presentationSummaryLogged = false;
        };

        uint64_t scheduleMpegPresentationVsync(
            MpegPlaybackState &playback,
            const MpegDecodedFrame &frame,
            uint64_t currentVsync)
        {
            if (frame.frameRateNumerator == 0u || frame.frameRateDenominator == 0u)
            {
                return currentVsync;
            }

            // MOH Frontline is NTSC.  Pace pictures against the emulated guest
            // field clock (60000/1001 Hz), using the MPEG sequence rate and the
            // decoded picture's repeat-field metadata.  No host wall clock or
            // frame dropping is involved.  For these progressive 30000/1001
            // streams, repeat_pict=0 and this resolves to exactly two VSyncs.
            constexpr uint64_t kNtscFieldRateNumerator = 60000u;
            constexpr uint64_t kNtscFieldRateDenominator = 1001u;

            uint64_t targetVsync = currentVsync;
            if (!playback.presentationPacingStarted)
            {
                playback.presentationPacingStarted = true;
                playback.presentationPhase = 0u;
                playback.firstPresentationVsync = targetVsync;
            }
            else
            {
                // If decode/refill was late, resume from the current guest
                // field instead of rapidly draining frames to "catch up".
                targetVsync = std::max(playback.nextPresentationVsync, currentVsync);
            }

            const uint64_t durationFields = 2u + static_cast<uint64_t>(frame.repeatPict);
            const uint64_t phaseIncrement =
                kNtscFieldRateNumerator *
                static_cast<uint64_t>(frame.frameRateDenominator) *
                durationFields;
            const uint64_t phaseDenominator =
                kNtscFieldRateDenominator *
                static_cast<uint64_t>(frame.frameRateNumerator) *
                2u;
            playback.presentationPhase += phaseIncrement;
            uint64_t fieldIntervals = playback.presentationPhase / phaseDenominator;
            playback.presentationPhase %= phaseDenominator;
            fieldIntervals = std::max<uint64_t>(fieldIntervals, 1u);
            playback.nextPresentationVsync = targetVsync + fieldIntervals;
            playback.lastPresentationVsync = targetVsync;
            return targetVsync;
        }

        void logMpegPresentationSummary(
            uint32_t mpegAddr,
            uint64_t generation,
            MpegPlaybackState &playback)
        {
            if (playback.presentationSummaryLogged ||
                !playback.presentationPacingStarted ||
                playback.picturesServed == 0u)
            {
                return;
            }

            playback.presentationSummaryLogged = true;
            const MpegDecodedFrame &timingFrame = playback.lastFrame;
            std::cerr << "[MOH:movie-pacing-summary]"
                      << " generation=" << generation
                      << " mpeg=0x" << std::hex << mpegAddr << std::dec
                      << " pictures=" << playback.picturesServed
                      << " firstVsync=" << playback.firstPresentationVsync
                      << " lastVsync=" << playback.lastPresentationVsync
                      << " fieldSpan=" << (playback.lastPresentationVsync - playback.firstPresentationVsync)
                      << " rate=" << timingFrame.frameRateNumerator << "/" << timingFrame.frameRateDenominator
                      << " repeatPict=" << timingFrame.repeatPict
                      << " streamEnded=" << (playback.streamEnded ? 1u : 0u)
        ...
        void runGuestMpegCallback(uint8_t *rdram,
                                  ...
                                  uint32_t a3)
        {
            ...
            SET_GPR_U32(&callbackCtx, 4, a0);
            SET_GPR_U32(&callbackCtx, 5, a1);
            SET_GPR_U32(&callbackCtx, 6, a2);
            SET_GPR_U32(&callbackCtx, 7, a3);
                                         ...
                                         const MpegRegisteredCallback &callback)
        {
            ...
            runGuestMpegCallback(rdram, callerCtx, runtime, callback,
                                 event.mpegAddr, cbDataAddr, callback.data, 0u);

            runtime->guestFree(cbDataAddr);
        }

        bool dispatchNoDataCallbacks(uint8_t *rdram,
                                     R5900Context *ctx,
                                     PS2Runtime *runtime,
                                     uint32_t mpegAddr)
        {
            std::vector<MpegRegisteredCallback> callbacks;
            {
                    ...
                    getPlaybackState(mpegAddr).decodedFrames.empty())
                {
                    for (const MpegRegisteredCallback &callback : it->second)
                    {
                        if (!callback.stream && callback.type == 1u)
                        {
            ...
            // sceMpegGetPicture is entered while the current EE thread owns the
            // guest-execution lock.  The guest's no-data callback services the
            // sound queue and can wait for another guest thread to retire a slot,
            // so holding the outer lock here starves the very worker it needs.
            // Stream callbacks already use the same unlocked dispatch contract.
            PS2Runtime::GuestExecutionReleaseScope releaseGuestExecution(runtime);
            for (const MpegRegisteredCallback &callback : callbacks)
            {
                static std::atomic<uint32_t> s_dispatchCount{0u};
                const uint32_t index = s_dispatchCount.fetch_add(1u, std::memory_order_relaxed);
                if (index < 64u)
                {
                    std::cerr << "[MPEG:nodata-callback] #" << index
                              << " mpeg=0x" << std::hex << mpegAddr
                              << " cb=0x" << callback.func
                              << " data=0x" << callback.data
                              << std::dec << std::endl;
                }
                runGuestMpegCallback(rdram, ctx, runtime, callback,
                                     mpegAddr, callback.data, 0u, 0u);
            }
            return true;
        }
    ...
    {
        std::lock_guard<std::mutex> lock(g_mpeg_stub_mutex);
        const uint64_t previousGeneration = g_mpeg_stub_state.cdStreamGeneration;
        for (auto &[mpegAddr, playback] : g_mpeg_stub_state.playbackByMpeg)
        {
            logMpegPresentationSummary(mpegAddr, previousGeneration, playback);
        }
    ...
    void sceMpegAddBs(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        playback.bytesSubmitted += copied;
    ...
    {
        traceMpegStubCall("AddCallback", ctx);
        (void)rdram;
        (void)runtime;
    ...
    {
        traceMpegStubCall("DemuxPss", ctx);
        const uint32_t mpegAddr = getRegU32(ctx, 4);
        const uint32_t dataAddr = getRegU32(ctx, 5);
        const uint32_t byteCount = getRegU32(ctx, 6);
    ...
    {
        traceMpegStubCall("DemuxPssRing", ctx);
    ...
    {
        traceMpegStubCall("GetPicture", ctx);
        const uint32_t mpegAddr = getRegU32(ctx, 4);
        const uint32_t imageAddr = getRegU32(ctx, 5);
        for (uint32_t refill = 0u; refill < 32u; ++refill)
        {
            ...
            bool haveFrame = false;
            {
                ...
                bytesBefore = playback.bytesSubmitted;
                haveFrame = !playback.decodedFrames.empty();
            }
            if (haveFrame || !dispatchNoDataCallbacks(rdram, ctx, runtime, mpegAddr))
            {
                break;
            }
            ...
            std::lock_guard<std::mutex> lock(g_mpeg_stub_mutex);
            const MpegPlaybackState &playback = getPlaybackState(mpegAddr);
            if (!playback.decodedFrames.empty() || playback.bytesSubmitted == bytesBefore)
            {
                break;
            }
        }
        ...
        uint64_t presentationTargetVsync = 0u;
        uint64_t presentationGeneration = 0u;
        size_t presentationQueuedFrames = 0u;
        uint32_t actualFrameTraceIndex = std::numeric_limits<uint32_t>::max();
        {
            PS2Runtime::GuestExecutionReleaseScope releaseGuestExecution(runtime);
            ...
            if (!playback.decodedFrames.empty())
            {
                ...
                haveFrame = true;
                static std::atomic<uint32_t> s_actualFrameHashCount{0u};
                const uint32_t hashIndex = s_actualFrameHashCount.fetch_add(1u, std::memory_order_relaxed);
                actualFrameTraceIndex = hashIndex;
                if (hashIndex < 192u)
                {
                    ...
                    std::cerr << "[MPEG:GetPicture:actual-frame] #" << hashIndex
                              << " mpeg=0x" << std::hex << mpegAddr
                              << " hash=0x" << hash
                              << std::dec << " generation=" << g_mpeg_stub_state.cdStreamGeneration
                              << " frame=" << frameCount
                              << " size=" << width << "x" << height
                              << " bytes=" << frame.rgba.size()
                              << std::endl;
                }
            ...
            if (haveFrame)
            {
                presentationTargetVsync = scheduleMpegPresentationVsync(
                    playback,
                    frame,
                    ps2_syscalls::GetCurrentVSyncTick());
                presentationGeneration = g_mpeg_stub_state.cdStreamGeneration;
                presentationQueuedFrames = playback.decodedFrames.size();
            }
        }

        if (haveFrame && frame.frameRateNumerator != 0u && frame.frameRateDenominator != 0u)
        {
            while (runtime &&
                   !runtime->isStopRequested() &&
                   ps2_syscalls::GetCurrentVSyncTick() < presentationTargetVsync)
            {
                (void)ps2_syscalls::WaitForNextVSyncTick(rdram, runtime);
            }
        }

        if (actualFrameTraceIndex < 2048u)
        {
            static const auto s_moviePacingEpoch = std::chrono::steady_clock::now();
            uint32_t guest100Hz = 0u;
            if (uint8_t *clockPtr = getMemPtr(rdram, 0x00331EE8u))
            {
                std::memcpy(&guest100Hz, clockPtr, sizeof(guest100Hz));
            }
            const auto hostUs = std::chrono::duration_cast<std::chrono::microseconds>(
                                    std::chrono::steady_clock::now() - s_moviePacingEpoch)
                                    .count();
            std::cerr << "[MOH:movie-pacing]"
                      << " index=" << actualFrameTraceIndex
                      << " generation=" << presentationGeneration
                      << " frame=" << frameCount
                      << " vsync=" << ps2_syscalls::GetCurrentVSyncTick()
                      << " targetVsync=" << presentationTargetVsync
                      << " guest100=" << guest100Hz
                      << " hostUs=" << hostUs
                      << " queued=" << presentationQueuedFrames
                      << " rate=" << frame.frameRateNumerator << "/" << frame.frameRateDenominator
        ...
        static std::atomic<uint32_t> s_getPictureResultCount{0u};
        const uint32_t resultIndex = s_getPictureResultCount.fetch_add(1u, std::memory_order_relaxed);
        if (resultIndex < 192u)
        {
            uint64_t bytesSubmitted = 0u;
            size_t queuedFrames = 0u;
            {
                std::lock_guard<std::mutex> lock(g_mpeg_stub_mutex);
                const MpegPlaybackState &playback = getPlaybackState(mpegAddr);
                bytesSubmitted = playback.bytesSubmitted;
                queuedFrames = playback.decodedFrames.size();
            }
            std::cerr << "[MPEG:GetPicture:result] #" << resultIndex
                      << " mpeg=0x" << std::hex << mpegAddr
                      << " image=0x" << imageAddr
                      << std::dec << " haveFrame=" << (haveFrame ? 1u : 0u)
                      << " frame=" << frameCount
                      << " size=" << width << "x" << height
                      << " queued=" << queuedFrames
    ...
    void sceMpegIsEnd(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (ended && playback.decodedFrames.empty())
        {
            logMpegPresentationSummary(
                mpegAddr,
                g_mpeg_stub_state.cdStreamGeneration,
                playback);
        }
