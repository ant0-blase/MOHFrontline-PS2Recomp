...
namespace ps2_stubs
{
    namespace
    {
        ...
        void logMohMcCall(const char *name, const R5900Context *ctx, int32_t port = -1,
                          int32_t slot = -1, const std::string &path = {})
        {
            static std::unordered_map<std::string, uint32_t> counts;
            const std::string key = name ? name : "sceMc?";
    ...
    void sceMcGetDir(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // PS2 EABI: the 5th/6th args (maxent, table*) are in t0/t1 ($8/$9), not on the
        // stack. Reading them from the stack gave a garbage count + table pointer, so the
        // directory was filled wrong and the EA save-manager's entry loop (sub_0021AFF8 @
        // 0x21b650) spun forever. Prefer t0/t1, fall back to stack for o32 callers.
        uint32_t maxEntriesReg = getRegU32(ctx, 8);
        if (maxEntriesReg == 0u) maxEntriesReg = readStackU32(rdram, ctx, 16);
        const int32_t maxEntries = static_cast<int32_t>(maxEntriesReg);
        uint32_t tableAddr = getRegU32(ctx, 9);
        if (tableAddr == 0u) tableAddr = readStackU32(rdram, ctx, 20);
        logMohMcCall("sceMcGetDir", ctx, port, slot, rawPath);
...
// === MOH Frontline memory-card bypass ===
// Controlled by PS2_MOH_BYPASS_MEMCARD=1
static bool mohMcBypassEnabled()
{
    const char *v = std::getenv("PS2_MOH_BYPASS_MEMCARD");
    return v && v[0] && v[0] != '0';
}
...
template <typename T>
static uint32_t mohMcGpr32Impl(T *ctx, int reg)
{
    if constexpr (requires { ctx->gpr[0].u64[0]; }) {
        return static_cast<uint32_t>(ctx->gpr[reg].u64[0]);
    } else if constexpr (requires { ctx->gpr[0].u64; }) {
        return static_cast<uint32_t>(ctx->gpr[reg].u64);
    } else if constexpr (requires { ctx->gpr[0].u32[0]; }) {
        return static_cast<uint32_t>(ctx->gpr[reg].u32[0]);
    } else if constexpr (requires { ctx->r[0].u64[0]; }) {
        return static_cast<uint32_t>(ctx->r[reg].u64[0]);
    } else if constexpr (requires { ctx->r[0].u64; }) {
        return static_cast<uint32_t>(ctx->r[reg].u64);
    } else if constexpr (requires { ctx->r[0].u32[0]; }) {
        return static_cast<uint32_t>(ctx->r[reg].u32[0]);
    } else if constexpr (requires { ctx->regs[0].u64[0]; }) {
        return static_cast<uint32_t>(ctx->regs[reg].u64[0]);
    } else if constexpr (requires { ctx->regs[0].u64; }) {
        return static_cast<uint32_t>(ctx->regs[reg].u64);
    } else if constexpr (requires { ctx->regs[0].u32[0]; }) {
        return static_cast<uint32_t>(ctx->regs[reg].u32[0]);
    } else if constexpr (requires { _mm_cvtsi128_si64(ctx->gpr[0]); }) {
        return static_cast<uint32_t>(_mm_cvtsi128_si64(ctx->gpr[reg]));
    } else if constexpr (requires { _mm_cvtsi128_si64(ctx->r[0]); }) {
        return static_cast<uint32_t>(_mm_cvtsi128_si64(ctx->r[reg]));
    } else if constexpr (requires { _mm_cvtsi128_si64(ctx->regs[0]); }) {
        return static_cast<uint32_t>(_mm_cvtsi128_si64(ctx->regs[reg]));
    } else if constexpr (requires { ctx->cpuRegs.GPR.r[0].UD[0]; }) {
        return static_cast<uint32_t>(ctx->cpuRegs.GPR.r[reg].UD[0]);
    } else {
        static_assert(sizeof(T) == 0, "Unknown R5900Context GPR layout for mohMcGpr32Impl");
        return 0;
    }
}

static uint32_t mohMcGpr32(R5900Context *ctx, int reg)
{
    return mohMcGpr32Impl(ctx, reg);
}

template <typename T>
static void mohMcSetGpr32Impl(T *ctx, int reg, uint32_t value)
{
    if constexpr (requires { ctx->gpr[0].u64[0] = uint64_t{}; }) {
        ctx->gpr[reg].u64[0] = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->gpr[0].u64 = uint64_t{}; }) {
        ctx->gpr[reg].u64 = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->gpr[0].u32[0] = uint32_t{}; }) {
        ctx->gpr[reg].u32[0] = value;
    } else if constexpr (requires { ctx->r[0].u64[0] = uint64_t{}; }) {
        ctx->r[reg].u64[0] = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->r[0].u64 = uint64_t{}; }) {
        ctx->r[reg].u64 = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->r[0].u32[0] = uint32_t{}; }) {
        ctx->r[reg].u32[0] = value;
    } else if constexpr (requires { ctx->regs[0].u64[0] = uint64_t{}; }) {
        ctx->regs[reg].u64[0] = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->regs[0].u64 = uint64_t{}; }) {
        ctx->regs[reg].u64 = static_cast<uint64_t>(value);
    } else if constexpr (requires { ctx->regs[0].u32[0] = uint32_t{}; }) {
        ctx->regs[reg].u32[0] = value;
    } else if constexpr (requires { ctx->gpr[0] = _mm_cvtsi64_si128(0); }) {
        ctx->gpr[reg] = _mm_cvtsi64_si128(static_cast<long long>(value));
    } else if constexpr (requires { ctx->r[0] = _mm_cvtsi64_si128(0); }) {
        ctx->r[reg] = _mm_cvtsi64_si128(static_cast<long long>(value));
    } else if constexpr (requires { ctx->regs[0] = _mm_cvtsi64_si128(0); }) {
        ctx->regs[reg] = _mm_cvtsi64_si128(static_cast<long long>(value));
    } else if constexpr (requires { ctx->cpuRegs.GPR.r[0].UD[0] = uint64_t{}; }) {
        ctx->cpuRegs.GPR.r[reg].UD[0] = static_cast<uint64_t>(value);
    } else {
        static_assert(sizeof(T) == 0, "Unknown R5900Context GPR layout for mohMcSetGpr32Impl");
    }
}

static void mohMcSetV0(R5900Context *ctx, uint32_t value)
{
    mohMcSetGpr32Impl(ctx, 2, value);
}


static void mohMcWrite32(uint8_t *rdram, uint32_t addr, uint32_t value)
{
    if (!addr) return;
    uint32_t phys = addr & 0x01ffffffu;
    std::memcpy(rdram + phys, &value, sizeof(value));
}

static uint32_t mohMcRead32(uint8_t *rdram, uint32_t addr)
{
    if (!addr) return 0;
    uint32_t phys = addr & 0x01ffffffu;
    uint32_t value = 0;
    std::memcpy(&value, rdram + phys, sizeof(value));
    return value;
}

static bool g_mohMcBypassPending = false;
static int g_mohMcBypassPort = 0;
static int g_mohMcBypassSlot = 0;

void sceMcGetInfo(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {

    if (mohMcBypassEnabled())
    {
        const uint32_t port = mohMcGpr32(ctx, 4);
        const uint32_t slot = mohMcGpr32(ctx, 5);
        const uint32_t typePtr = mohMcGpr32(ctx, 6);
        const uint32_t freePtr = mohMcGpr32(ctx, 7);
        const uint32_t sp = mohMcGpr32(ctx, 29);
        // PS2 EABI: 5th arg (format*) is in t0 ($8), not on the stack.
        uint32_t formatPtr = mohMcGpr32(ctx, 8);
        if (formatPtr == 0u) formatPtr = mohMcRead32(rdram, sp + 0x10u);

        // Simule une Memory Card PS2 formatée avec assez d'espace.
        mohMcWrite32(rdram, typePtr, 2);       // PS2 memory card
        mohMcWrite32(rdram, freePtr, 8192);    // largement > 149 KB
        mohMcWrite32(rdram, formatPtr, 1);     // formatted

        g_mohMcBypassPending = true;
        g_mohMcBypassPort = static_cast<int>(port);
        g_mohMcBypassSlot = static_cast<int>(slot);

        std::fprintf(stderr,
            "[MOH:memcard-bypass] sceMcGetInfo port=%u slot=%u typePtr=0x%x freePtr=0x%x formatPtr=0x%x ret=0\n",
            port, slot, typePtr, freePtr, formatPtr);
        ...
        // PS2 EABI passes the 5th arg (format*) in t0 ($8), not on the stack.
        // MOH's EA save-manager (FUN_001df9e0) reads the format flag at [a0+0x1C]
        // via t0 and bails to the memcard prompt if it stays 0; the prior stack
        // read left it unwritten. Prefer t0, fall back to stack for o32 callers.
        uint32_t formatPtr = getRegU32(ctx, 8);
        if (formatPtr == 0u) formatPtr = readStackU32(rdram, ctx, 16);
        logMohMcCall("sceMcGetInfo", ctx, port, slot);
        ...
        // PS2_MOH_MEMCARD_ABSENT=1: report "no card" like a console with an empty
        // slot. MOH's boot dialog probe (0x1DF938) treats type==2/result>=-10 as
        // "card present"; a present card freezes the caution text animation at
        // its initial state, while an absent card shows the expected
        // "Caution! ... insert a memory card ... press X" warning text.
        static const bool s_mohMemcardAbsent = [] {
            const char *v = std::getenv("PS2_MOH_MEMCARD_ABSENT");
            return v && v[0] && v[0] != '0';
        }();
        if (s_mohMemcardAbsent)
        {
            {
                std::lock_guard<std::mutex> lock(g_mcStateMutex);
                setMcCommandResultLocked(kMcCmdGetInfo, kMcResultNoEntry);
            }
            if (typePtr != 0u)
            {
...

        {
            ...
            // MOH's memcard probe treats the async GetInfo result as the card type.
            // Keep sceMcGetInfo's immediate return as success/request-accepted below,
            // but let sceMcSync report a PS2 card for the completed GetInfo command.
            const int32_t syncResult = (result == kMcResultSucceed) ? kMcTypePs2 : result;
            setMcCommandResultLocked(kMcCmdGetInfo, syncResult);
        }
    ...
    void sceMcOpen(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        logMohMcCall("sceMcOpen", ctx, port, slot, path);
    ...
    if (mohMcBypassEnabled() && g_mohMcBypassPending)
    {
        const uint32_t cmdPtr = mohMcGpr32(ctx, 5);
        const uint32_t resultPtr = mohMcGpr32(ctx, 6);

        mohMcWrite32(rdram, cmdPtr, 0);
        mohMcWrite32(rdram, resultPtr, 0);

        std::fprintf(stderr,
            "[MOH:memcard-bypass] sceMcSync port=%d slot=%d cmdPtr=0x%x resultPtr=0x%x result=0 ret=0\n",
            g_mohMcBypassPort, g_mohMcBypassSlot, cmdPtr, resultPtr);

        g_mohMcBypassPending = false;
        mohMcSetV0(ctx, 0);
        return;
    }
