    ...
    void strcpy(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
{
    ...
    setReturnS32(ctx, static_cast<int32_t>(dstAddr));

    if (dstAddr == 0u || srcAddr == 0u)
    {
        return;
    }

    auto isReasonableChar = [](uint8_t c) -> bool
    {
    ...
    static constexpr uint32_t kMaxString = 0x400u;

    uint32_t copied = 0;

    for (; copied < kMaxString; ++copied)
    {
        uint8_t *src = getMemPtr(rdram, srcAddr + copied);
        uint8_t *dst = getMemPtr(rdram, dstAddr + copied);
    ...
    uint8_t *term = getMemPtr(rdram, dstAddr + kMaxString - 1u);
    if (term)
    {
        *term = 0;
    }
    ...
    if (warnCount < 4)
    {
        std::cerr << "[strcpy:path-safe] forced terminator"
                  << " dst=0x" << std::hex << dstAddr
                  << " src=0x" << srcAddr
                  << std::dec << std::endl;
        ++warnCount;
    ...
    void strcat(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
{
    ...
    uint32_t dstLen = 0;
    bool dstTerminated = false;

    for (; dstLen < kMaxString; ++dstLen)
    {
        uint8_t *dst = getMemPtr(rdram, dstAddr + dstLen);
        if (!dst)
        {
            return;
        }
    ...
    if (!dstTerminated)
    {
        uint8_t *dst0 = getMemPtr(rdram, dstAddr);
        if (!dst0)
        {
            return;
        }
        ...
        *dst0 = 0;
        ...
        if (initCount < 4)
        {
            std::cerr << "[strcat:path-safe] initialized unterminated dst"
                      << " dst=0x" << std::hex << dstAddr
                      << " src=0x" << srcAddr
                      << std::dec << std::endl;
            ++initCount;
    ...
    for (; copied < kMaxString - dstLen; ++copied)
    {
        uint8_t *src = getMemPtr(rdram, srcAddr + copied);
        uint8_t *dst = getMemPtr(rdram, dstAddr + dstLen + copied);
