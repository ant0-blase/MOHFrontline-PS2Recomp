...
// MOH: defined at global scope in Sync.cpp (last guest-created sema id).
extern std::atomic<int> g_mohLastCreatedSemaId;
// MOH: per-thread "0x8000059a just completed" flag consumed by WaitSema (Sync.cpp).
extern std::atomic<bool> g_moh59aJustCompleted;


// [MOH] IOP snddrv service ids (from the removed runtime/ps2_iop.h).
...
namespace ps2_syscalls
{
    namespace
    {
        ...
        constexpr uint32_t kMohShellVivLbn = 0x000FB25Cu;
        constexpr uint32_t kMohShellVivObjectPtrAddr = 0x0024174Cu;
        constexpr uint32_t kMohShellVivObjectSizeAddr = 0x00241754u;
        constexpr uint32_t kMohShellVivFileEntryAddr = 0x01F8B62Cu;
        constexpr uint32_t kMohShellVivHash = 0x0E924DF4u;

        constexpr uint32_t kMohShell1AsfFileEntryAddr = 0x01F71690u;
        constexpr uint32_t kMohShell1AsfHash = 0x04150324u;
        constexpr uint32_t kMohShell1AsfLbn = 0x000FD2EBu;
        constexpr uint32_t kMohShell1AsfSize = 0x0105EDD4u;
        constexpr uint32_t kMohFrontendPumpFlagsAddr = 0x0025AB10u;
        constexpr uint32_t kMohFrontendPumpReadyBits = 0x00000006u;

        std::filesystem::path resolveMohCdFile(const char *ps2Path)
        {
            const std::filesystem::path translated = translatePs2Path(ps2Path);
            std::error_code ec;
            if (!translated.empty() && std::filesystem::exists(translated, ec) && !ec)
            {
                return translated;
            }

            std::string suffix(ps2Path ? ps2Path : "");
            const std::string lower = toLowerAscii(suffix);
            ...
            suffix = normalizePs2PathSuffix(std::move(suffix));

            const std::filesystem::path candidates[] = {
                getConfiguredCdRoot() / suffix,
                std::filesystem::path("extracted") / suffix,
            };
            ...
            for (const std::filesystem::path &candidate : candidates)
            {
                ...
                if (std::filesystem::exists(candidate, ec) && !ec)
                {
                    ...
                    const std::filesystem::path abs = std::filesystem::absolute(candidate, absEc);
                    return absEc ? candidate.lexically_normal() : abs.lexically_normal();
                }
            }
        ...
        bool readHostFileIntoGuest(const std::filesystem::path &path,
                                   uint8_t *rdram,
                                   uint32_t guestAddr,
                                   uint32_t expectedSize)
        {
            const uint32_t guestOffset = guestAddr & PS2_RAM_MASK;
            if (guestOffset >= PS2_RAM_SIZE || expectedSize > (PS2_RAM_SIZE - guestOffset))
            {
                return false;
            }

            uint8_t *dst = getMemPtr(rdram, guestAddr);
            if (!dst)
            {
                return false;
            }
            ...
            const uint32_t bytesToRead = std::min<uint32_t>(expectedSize, static_cast<uint32_t>(fileSize));
            file.seekg(0, std::ios::beg);
            std::memset(dst, 0, expectedSize);
            file.read(reinterpret_cast<char *>(dst), static_cast<std::streamsize>(bytesToRead));
            return file.gcount() == static_cast<std::streamsize>(bytesToRead);
        }

        void writeMohFileTableEntry(uint8_t *rdram,
                                    uint32_t entryAddr,
                                    uint32_t hashOrToken,
                                    uint32_t lbn,
                                    uint32_t size,
                                    const char *path)
        {
            (void)writeGuestU32(rdram, entryAddr + 0x00u, hashOrToken);
            (void)writeGuestU32(rdram, entryAddr + 0x04u, lbn);
            (void)writeGuestU32(rdram, entryAddr + 0x08u, size);

            if (uint8_t *pathDst = getMemPtr(rdram, entryAddr + 0x0Cu))
            {
                std::memset(pathDst, 0, 0x100u);
                const size_t pathLen = std::min<size_t>(std::strlen(path), 0xFFu);
                std::memcpy(pathDst, path, pathLen);
            }
        }

        bool ensureMohFrontendPayloadLoaded(uint8_t *rdram, PS2Runtime *runtime)
        {
            static std::mutex s_mutex;
            static uintptr_t s_owner = 0u;
            static bool s_attempted = false;
            static bool s_loaded = false;

            std::lock_guard<std::mutex> lock(s_mutex);

            const uintptr_t owner = reinterpret_cast<uintptr_t>(runtime);
            if (s_owner != owner)
            {
                s_owner = owner;
                s_attempted = false;
            ...
            const std::filesystem::path shellVivPath =
                resolveMohCdFile("cdrom0:\\DATA\\SHELL\\SHELL.VIV;1");
            if (shellVivPath.empty())
            {
                std::cerr << "[MOH:frontend-preload] SHELL.VIV not found under cd root" << std::endl;
                return false;
            }

            if (!readHostFileIntoGuest(shellVivPath, rdram, kMohShellVivGuestAddr, kMohShellVivSize))
            {
                std::cerr << "[MOH:frontend-preload] failed reading " << shellVivPath.string()
                          << " to guest 0x" << std::hex << kMohShellVivGuestAddr
                          << " size=0x" << kMohShellVivSize << std::dec << std::endl;
                return false;
            }

            (void)writeGuestU32(rdram, kMohShellVivObjectPtrAddr, kMohShellVivGuestAddr);
            (void)writeGuestU32(rdram, kMohShellVivObjectPtrAddr + 0x04u, 0u);
            (void)writeGuestU32(rdram, kMohShellVivObjectSizeAddr, kMohShellVivSize);

            writeMohFileTableEntry(rdram,
                                   kMohShellVivFileEntryAddr,
                                   kMohShellVivHash,
                                   kMohShellVivLbn,
                                   kMohShellVivSize,
                                   "/DATA/SHELL/SHELL.VIV");
            writeMohFileTableEntry(rdram,
                                   kMohShell1AsfFileEntryAddr,
                                   kMohShell1AsfHash,
                                   kMohShell1AsfLbn,
                                   kMohShell1AsfSize,
                                   "/DATA/SHELL/SHELL1.ASF");

            s_loaded = true;
            std::cerr << "[MOH:frontend-preload] SHELL.VIV -> guest=0x" << std::hex
                      << kMohShellVivGuestAddr
                      << " size=0x" << kMohShellVivSize
                      << " ptrSlot=0x" << kMohShellVivObjectPtrAddr
                      << " entry=0x" << kMohShellVivFileEntryAddr
                      << std::dec << std::endl;
            return true;
        }
        ...
        bool signalRpcCompletionSema(uint32_t semaId)
        {
            ...
            ps2sched::FiberToken wokenToken{};
            {
                ...
                if (!sema->deleted && sema->count < sema->maxCount)
                {
                    sema->count++;
                    signaled = true;
                    // [PR137] fiber wake protocol (mirror of SignalSema):
                    // pop one published waiter and deliver a validated wakeup.
    ...
    void SifInitRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (runtime)
        {
            // [PR137] IOP subsystem initializes in PS2Runtime::loadELF.
        }
    ...
    void SifCallRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        {
            // [MOH trace] capture which IOP RPC service/fno the game calls (find the file service).
            static std::atomic<uint32_t> s_mohRpcTrace{0};
            if (s_mohRpcTrace.fetch_add(1, std::memory_order_relaxed) < 360)
            {
                std::cerr << "[SifCallRpc:MOH-trace] sid=0x" << std::hex << sid
                          << " fno=0x" << rpcNum
                          << " sendSize=0x" << sendSize << " recvSize=0x" << recvSize
                          << " sendBuf=0x" << sendBuf << " recvBuf=0x" << recvBuf
                          << std::dec << std::endl;
            }
            // Dump the full request for the custom MOH file services a few times.
            static std::atomic<uint32_t> s_mohFileDump{0};
            if (sid >= 0x80000901u && sid <= 0x80000905u &&
                s_mohFileDump.fetch_add(1, std::memory_order_relaxed) < 48)
            {
                std::cerr << "[SifCallRpc:MOH-file-req] sid=0x" << std::hex << sid
                          << " fno=0x" << rpcNum
                          << " sendSize=0x" << sendSize
                          << " recvSize=0x" << recvSize
                          << " words:";
                if (const uint8_t *sb = getConstMemPtr(rdram, sendBuf))
                {
                    for (uint32_t i = 0; i < sendSize && i < 0x80u; i += 4)
                    {
                        uint32_t w = 0;
                        std::memcpy(&w, sb + i, sizeof(w));
                        std::cerr << " " << w;
                    }
                    std::cerr << "  ascii:\"";
                    for (uint32_t i = 0; i < sendSize && i < 0x80u; ++i)
                    {
                        char c = static_cast<char>(sb[i]);
                        std::cerr << ((c >= 0x20 && c < 0x7f) ? c : '.');
                    }
                    std::cerr << "\"";
                }
        ...
        {
            // [PR137] IOP subsystem initializes in PS2Runtime::loadELF.
            ps2x::iop::RpcRequest iopRequest{};
            iopRequest.sid = sid;
            iopRequest.function = rpcNum;
            iopRequest.mode = mode;
            iopRequest.send = {sendBuf, sendSize};
            iopRequest.receive = {recvBuf, recvSize};
            iopRequest.endFunction = endFunc;
            iopRequest.endParameter = endParam;
            const ps2x::iop::RpcResult iopResult =
                runtime->handleIopRpc(rdram, ctx, iopRequest);
            if (iopResult.handled)
            {
                handled = true;
                handledByIop = true;
                resultPtr = iopResult.resultAddress;
                if (iopResult.signalNowaitCompletion && (mode & kSifRpcModeNowait) != 0u)
                {
        ...
        // MOH Frontline IOP file-service handshake. The core binds servers 0x80000901..0x80000905
        // (FUN_00236f78) and version-checks 0x80000905 fno=1: it reads response word0 and requires
        // (word0 >> 8) == 3 to proceed. Answer with a v3 version so the handshake passes.
        if (!handled && sid == 0x80000905u && rpcNum == 1u && recvBuf && recvSize >= 4u)
        {
            rpcZeroRdram(rdram, recvBuf, recvSize);
            (void)writeRpcU32(recvBuf, 0x00000300u); // (0x300 >> 8) == 3
            handled = true;
            resultPtr = recvBuf;
            static uint32_t s_mohVerLog = 0;
            if (s_mohVerLog++ < 4)
            {
                std::cerr << "[MOH:rpc] 0x80000905 fno=1 version handshake -> word0=0x300" << std::endl;
            }
        }

        // MOH file service status polls. PCSX2 reaches the menu with SHELL.VIV resident at 0x00B0BF80;
        // mirror that state before answering ready so the generated frontend path can consume real data.
        if (!handled && rpcNum == 1u && recvBuf && recvSize >= 8u &&
            (sid == 0x80000901u || sid == 0x80000902u || sid == 0x80000903u))
        {
            const bool payloadReady = ensureMohFrontendPayloadLoaded(rdram, runtime);
            rpcZeroRdram(rdram, recvBuf, recvSize);
            const uint32_t ready = payloadReady ? 1u : 0u;
            (void)writeRpcU32(recvBuf + 0u, ready);
            (void)writeRpcU32(recvBuf + 4u, ready); // sub_002371F8/00237268/002372D8 return word1.
            handled = true;
            resultPtr = recvBuf;
            static uint32_t s_mohFileReadyLog = 0;
            if (s_mohFileReadyLog++ < 12)
            {
                std::cerr << "[MOH:rpc] sid=0x" << std::hex << sid
                          << " fno=1 -> word0/word1=" << std::dec << ready
                          << " shellViv=" << (payloadReady ? "loaded" : "missing")
                          << std::endl;
            }
        }

        // FUN_00237348 (0x80000904 fno=1) returns response word2. Keep it in lockstep with
        // the other MOH file-service readiness checks.
        if (!handled && sid == 0x80000904u && rpcNum == 1u && recvBuf && recvSize >= 12u)
        {
            const bool payloadReady = ensureMohFrontendPayloadLoaded(rdram, runtime);
            rpcZeroRdram(rdram, recvBuf, recvSize);
            const uint32_t ready = payloadReady ? 1u : 0u;
            (void)writeRpcU32(recvBuf + 8u, ready);
            handled = true;
            resultPtr = recvBuf;
            static uint32_t s_moh904Log = 0;
            if (s_moh904Log++ < 8)
            {
                std::cerr << "[MOH:rpc] 0x80000904 fno=1 -> word2=" << ready
                          << " shellViv=" << (payloadReady ? "loaded" : "missing")
                          << std::endl;
            }
        }

        // MOH EA sound-stream poll RPC (sid 0x8000059a, send/recv one word). No handler
        // filled recvBuf, so the game read STALE data -> movie decode started only when the
        // leftover bytes happened to look "ready" (nondeterministic across runs). Answer
        // deterministically; PS2_MOH_RPC59A_WORD overrides the response word for testing.
        if (!handled && sid == 0x8000059Au && recvBuf && recvSize >= 4u)
        {
            static const uint32_t s_word = [] {
                const char *v = std::getenv("PS2_MOH_RPC59A_WORD");
                // Default 0 = what the launcher's zeroed .bss recv buffer held originally.
                return v ? static_cast<uint32_t>(std::strtoul(v, nullptr, 0)) : 0u;
            }();
            rpcZeroRdram(rdram, recvBuf, recvSize);
            (void)writeRpcU32(recvBuf, s_word);
            handled = true;
            resultPtr = recvBuf;
            // The EA sound lib does CreateSema(init=0) -> SifCallRpc -> WaitSema and
            // relies on the RPC completion to signal that sema. Without the signal the
            // audio worker blocks forever and the movie player aborts each movie with
            // -2 after one update. Signal the client's completion sema like the other
            // handled-RPC paths do.
            {
                uint32_t semaId = static_cast<uint32_t>(client->hdr.sema_id);
                if (semaId == 0xFFFFFFFFu || semaId == 0u)
                {
                ...
                if (semaId == 0xFFFFFFFFu || semaId == 0u)
                {
                    // EA sound lib pattern: CreateSema(init=0) immediately before the
                    // RPC, WaitSema right after — signal the most recent sema.
                    const int lastSema = ::g_mohLastCreatedSemaId.load(std::memory_order_relaxed);
                    if (lastSema > 0)
                    {
                        semaId = static_cast<uint32_t>(lastSema);
                    }
                }
                const bool signaled = signalRpcCompletionSema(semaId);
                ::g_moh59aJustCompleted.store(true, std::memory_order_relaxed);
                static uint32_t s_moh59aLog = 0;
                if (s_moh59aLog++ < 6)
                {
                    std::cerr << "[MOH:rpc] 0x8000059a poll -> word0=" << s_word
                              << " sema=" << semaId << " signaled=" << (signaled ? 1 : 0)
                              << std::endl;
                }
            }
        }

        // MOH's custom SND RPC uses the same EE buffer for request/response during frontend init.
        // Return a clean success word so stale request data cannot be mistaken for a pending/error state.
        if (!handled && sid == 0x00534E44u && rpcNum == 0u && recvBuf && recvSize >= 4u)
        {
            uint32_t requestWords[5] = {};
            if (const uint8_t *sb = getConstMemPtr(rdram, sendBuf))
            {
                const uint32_t wordsToRead = std::min<uint32_t>(5u, sendSize / sizeof(uint32_t));
                for (uint32_t i = 0; i < wordsToRead; ++i)
                {
                    std::memcpy(&requestWords[i], sb + (i * sizeof(uint32_t)), sizeof(uint32_t));
                }
            }

            uint32_t oldPumpFlags = 0u;
            (void)readGuestU32(rdram, kMohFrontendPumpFlagsAddr, oldPumpFlags);
            const uint32_t newPumpFlags = oldPumpFlags | kMohFrontendPumpReadyBits;
            (void)writeGuestU32(rdram, kMohFrontendPumpFlagsAddr, newPumpFlags);

            rpcZeroRdram(rdram, recvBuf, recvSize);
            (void)writeRpcU32(recvBuf, 0u);
            handled = true;
            resultPtr = recvBuf;
            static uint32_t s_mohSndLog = 0;
            if (s_mohSndLog++ < 12)
            {
                std::cerr << "[MOH:rpc] SND fno=0 -> word0=0 req="
                          << std::hex << requestWords[0] << "," << requestWords[1]
                          << "," << requestWords[2] << "," << requestWords[3]
                          << "," << requestWords[4]
                          << " pumpFlags=0x" << oldPumpFlags << "->0x" << newPumpFlags
                          << std::dec << std::endl;
            }
        }
