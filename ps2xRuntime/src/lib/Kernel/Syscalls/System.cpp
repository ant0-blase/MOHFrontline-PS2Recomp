...
namespace ps2_syscalls
{
    ...
    void GetRomName(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        const uint32_t rawSize = getRegU32(ctx, 5); // $a1
        ...
        // Some games pass -1 / 0xffffffff as "large enough".
        // Never trust the guest size directly for host strncpy/memcpy.
        uint32_t bufSize = rawSize;
        static constexpr uint32_t kSafeRomNameBufferSize = 32u;

        if (bufSize > kSafeRomNameBufferSize)
        {
            bufSize = kSafeRomNameBufferSize;
        }

        const uint32_t romLen = static_cast<uint32_t>(std::strlen(romName));
        const uint32_t copyLen = std::min<uint32_t>(romLen, bufSize - 1u);

        for (uint32_t i = 0; i < copyLen; ++i)
        {
            uint8_t *dst = getMemPtr(rdram, bufAddr + i);
            if (!dst)
            {
                std::cerr << "GetRomName error: Invalid write address 0x"
                          << std::hex << (bufAddr + i) << std::dec << std::endl;
                setReturnS32(ctx, -1);
                return;
            }

            *dst = static_cast<uint8_t>(romName[i]);
        }

        uint8_t *nul = getMemPtr(rdram, bufAddr + copyLen);
        if (!nul)
        {
            std::cerr << "GetRomName error: Invalid terminator address 0x"
                      << std::hex << (bufAddr + copyLen) << std::dec << std::endl;
            setReturnS32(ctx, -1);
            return;
        }
        ...
        *nul = 0;
        ...
        setReturnS32(ctx, static_cast<int32_t>(romLen));
    }
