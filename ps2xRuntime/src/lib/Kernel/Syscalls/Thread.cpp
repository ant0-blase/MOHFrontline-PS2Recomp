...
namespace ps2_syscalls
{
    ...
    // [MOH diag] textual snapshot of guest thread states for A2/scheduler traces.
    std::string mohThreadStateSnapshotForTrace()
    {
        std::vector<std::pair<int, std::shared_ptr<ThreadInfo>>> threads;
        {
            ...
            for (const auto &item : g_threads)
            {
                threads.emplace_back(item.first, item.second);
            }
        }
        std::sort(
            threads.begin(),
        ...
        for (size_t index = 0u; index < threads.size(); ++index)
        {
            ...
            const int tid = threads[index].first;
            const std::shared_ptr<ThreadInfo> &info = threads[index].second;
            if (!info)
            {
                out << "tid=" << std::dec << tid << ",missing=1";
                continue;
            }

            const uint32_t pc =
                info->currentPc.load(std::memory_order_acquire);
            std::unique_lock<std::mutex> infoLock(
                info->m,
                std::try_to_lock);
            out << "tid=" << std::dec << tid
                << ",pc=0x" << std::hex << pc
                << ",locked=" << (infoLock.owns_lock() ? 1 : 0);
            if (infoLock.owns_lock())
            {
                out << ",status=0x" << info->status
                    << ",waitType=0x" << info->waitType
                    << ",waitId=0x" << info->waitId
                    << ",priority=0x" << info->currentPriority
                    << ",suspend=0x" << info->suspendCount
                    << ",wakeups=0x" << info->wakeupCount;
            }
        }
        out << ")";
        return out.str();
