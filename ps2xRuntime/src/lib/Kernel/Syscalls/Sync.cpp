...
// [MOH] cross-TU trackers consumed by the MOH RPC pass-through (RPC.cpp):
// last CreateSema id (the game binds RPC completion semas right after
// creating them) and the 0x8000059A just-completed latch.
std::atomic<int> g_mohLastCreatedSemaId{-1};
std::atomic<bool> g_moh59aJustCompleted{false};
...
namespace ps2_syscalls
{
    ...
    void CreateSema(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        g_mohLastCreatedSemaId.store(id, std::memory_order_relaxed);
        setReturnS32(ctx, id);
    }
    ...
    {
        // [MOH] Movie-open RPC wait (ra 0x1f2e00..0x1f2e80): the EA sound lib
        // waits on a sema whose completion callback is never executed (HLE RPC
        // completes synchronously). NOT a lost-wakeup — fibers cannot fix it.
        // A-posteriori detection proven under moh-port: on the SECOND block of
        // the same sema at this ra, force success. (The broad VFS loader-bypass
        // windows from moh-port are intentionally NOT ported: the fiber
        // scheduler's SignalSema wake protocol makes them unnecessary.)
        {
            const uint32_t mohRa = getRegU32(ctx, 31);
            if (mohRa >= 0x001f2e00u && mohRa <= 0x001f2e80u)
            {
                static std::map<uint32_t, uint32_t> s_mohMovieWaitBlocks;
                static std::mutex s_mohMovieWaitMx;
                std::lock_guard<std::mutex> lk(s_mohMovieWaitMx);
                const uint32_t waitSid = getRegU32(ctx, 4);
                if (++s_mohMovieWaitBlocks[waitSid] >= 2u)
                {
                    s_mohMovieWaitBlocks[waitSid] = 0u;
                    static uint32_t s_mohMovieBypassLogs = 0u;
                    if (s_mohMovieBypassLogs++ < 16u)
                    {
                        std::cerr << "[WaitSema:MOH-movie-rpc-bypass] sema=0x"
                                  << std::hex << waitSid << " ra=0x" << mohRa
                                  << std::dec << std::endl;
                    }
                    setReturnS32(ctx, 0);
                    return;
