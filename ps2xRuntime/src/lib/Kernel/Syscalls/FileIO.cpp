...
namespace ps2_syscalls
{
    ...
    static std::unordered_map<int, std::string> g_fioAssetPaths;

    static std::string sanitizeFioPathForLog(const std::string &value)
    {
        std::string out;
    ...
    static bool shouldTraceMohFioAssetPath(const std::string &path)
    {
    ...
    static void logMohFioAssetOnce(const char *tag, int fd, const std::string &ps2Path,
                                   const std::string &hostPath, const R5900Context *ctx,
                                   uint32_t bufAddr = 0u, size_t size = 0u)
    {
        if (!shouldTraceMohFioAssetPath(ps2Path) && !shouldTraceMohFioAssetPath(hostPath))
        {
            return;
        }

        static std::unordered_set<std::string> logged;
        const std::string key = std::string(tag) + ":" + std::to_string(fd) + ":" + hostPath;
        if (!logged.insert(key).second)
        {
            return;
        }

        std::cerr << "[fio:MOH-asset] " << tag
                  << " fd=" << fd
                  << " ps2=\"" << sanitizeFioPathForLog(ps2Path) << "\""
                  << " host=\"" << sanitizeFioPathForLog(hostPath) << "\"";
        if (bufAddr != 0u || size != 0u)
        {
            std::cerr << " buf=0x" << std::hex << bufAddr
                      << " size=0x" << size;
        }
        if (ctx)
        {
    ...
    void fioOpen(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        {
            // [MOH diag pt6h] broad open log to see if .bum / menu files route through the runtime.
            static std::atomic<uint32_t> s_openLog{0};
            if (s_openLog.fetch_add(1u, std::memory_order_relaxed) < 250u)
                std::cerr << "[MOH:fioOpen] ps2='" << ps2Path << "' host='" << hostPath
                          << "' ok=" << (fp ? 1 : 0) << std::endl;
        }
        if (!fp)
        {
        ...
        if (shouldTraceMohFioAssetPath(ps2Path) || shouldTraceMohFioAssetPath(hostPath))
        {
            g_fioAssetPaths[ps2Fd] = hostPath;
            logMohFioAssetOnce("open", ps2Fd, ps2Path, hostPath, ctx);
        }
    ...
    void fioRead(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (auto pathIt = g_fioAssetPaths.find(ps2Fd); pathIt != g_fioAssetPaths.end())
        {
            logMohFioAssetOnce("read", ps2Fd, pathIt->second, pathIt->second, ctx, bufAddr, bytesRead);
        }
