...
// [MOH] IOP sound-driver RPC state (consumed by RPC.cpp's MOH pass-through).
...
// [MOH] DTX debug flag (used by the DTX URPC trace path).
static constexpr uint32_t kSifRpcDebugFlagDtx = 1u << 9;

// [MOH] sound-driver + DTX RPC globals/structs (consumed by RPC.cpp).
