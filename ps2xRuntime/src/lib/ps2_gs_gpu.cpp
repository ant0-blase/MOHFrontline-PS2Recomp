...
namespace mohgeom
{
    // Defined in ps2_gif_arbiter.cpp: the GIF path currently being drained.
    extern thread_local uint32_t t_gifPathId;
    inline thread_local uint32_t *g_vtxSinceStrip = nullptr;

    thread_local uint32_t t_gifPacketIndex = 0xFFFFFFFFu;
    thread_local uint32_t t_gifPacketBytes = 0u;
    thread_local uint32_t t_gifByteOffset = 0xFFFFFFFFu;
    thread_local uint32_t t_gifTagIndex = 0u;
    thread_local uint32_t t_gifPrimitiveIndex = 0u;
}
...
namespace
{
    ...
    bool decodeBmp24ToRgbTriples(const std::vector<uint8_t> &bmp,
                                 ...
                                 std::vector<uint8_t> &rgb)
    {
        ...
        const uint32_t dataOffset = readLe32(bmp, 10u);
        const uint32_t dibSize = readLe32(bmp, 14u);
        const int32_t width = static_cast<int32_t>(readLe32(bmp, 18u));
        const int32_t signedHeight = static_cast<int32_t>(readLe32(bmp, 22u));
        const uint16_t planes = readLe16(bmp, 26u);
        const uint16_t bpp = readLe16(bmp, 28u);
        const uint32_t compression = readLe32(bmp, 30u);
        const uint32_t height = signedHeight < 0 ? static_cast<uint32_t>(-signedHeight)
                                                 : static_cast<uint32_t>(signedHeight);
        if (dibSize < 40u ||
            width <= 0 ||
            static_cast<uint32_t>(width) != expectedWidth ||
        ...
        const uint32_t rowStride = ((expectedWidth * 3u) + 3u) & ~3u;
        const uint64_t requiredSize = static_cast<uint64_t>(dataOffset) +
                                      static_cast<uint64_t>(rowStride) * expectedHeight;
        if (requiredSize > bmp.size())
        {
            return false;
        }

        rgb.assign(static_cast<size_t>(expectedWidth) * expectedHeight * 3u, 0u);
        for (uint32_t y = 0; y < expectedHeight; ++y)
        {
            const uint32_t sourceY = signedHeight < 0 ? y : (expectedHeight - 1u - y);
            const uint8_t *src = bmp.data() + dataOffset + (sourceY * rowStride);
            uint8_t *dst = rgb.data() + (static_cast<size_t>(y) * expectedWidth * 3u);
            for (uint32_t x = 0; x < expectedWidth; ++x)
            {
                dst[(x * 3u) + 0u] = src[(x * 3u) + 2u];
                dst[(x * 3u) + 1u] = src[(x * 3u) + 1u];
                dst[(x * 3u) + 2u] = src[(x * 3u) + 0u];
            }
        }
        return true;
    }

    const std::vector<uint8_t> &mohStartBmpPixels()
    {
        ...
        std::call_once(s_loadOnce, []
        {
            ...
            const std::array<std::filesystem::path, 4> candidates = {
                cwd / "extracted" / "DATA" / "START.BMP",
                cwd / "DATA" / "START.BMP",
                cwd / "data" / "start.bmp",
                std::filesystem::path("extracted") / "DATA" / "START.BMP"
            };
            ...
            for (const std::filesystem::path &candidate : candidates)
            {
                ...
                if (std::filesystem::is_regular_file(candidate, ec) && !ec &&
                    readWholeFile(candidate, bmp) &&
                    decodeBmp24ToRgbTriples(bmp, 512u, 256u, s_pixels))
                {
                    break;
                }
            }
    ...
    bool isMohStartBmpTransfer(uint32_t dbp,
                               uint8_t dbw,
                               uint8_t dpsm,
                               uint16_t rrw,
                               uint16_t rrh,
    ...
    constexpr uint32_t kNoMohGifPacket = 0xFFFFFFFFu;
    thread_local uint32_t g_mohCurrentGifPacket = kNoMohGifPacket;

    struct ScopedMohGifPacket
    {
        ScopedMohGifPacket(uint32_t packetIndex, uint32_t packetBytes)
        {
            g_mohCurrentGifPacket = packetIndex;
            mohgeom::t_gifPacketIndex = packetIndex;
            mohgeom::t_gifPacketBytes = packetBytes;
            mohgeom::t_gifByteOffset = 0xFFFFFFFFu;
            mohgeom::t_gifTagIndex = 0u;
            mohgeom::t_gifPrimitiveIndex = 0u;
        }

        ~ScopedMohGifPacket()
        {
            g_mohCurrentGifPacket = kNoMohGifPacket;
            mohgeom::t_gifPacketIndex = 0xFFFFFFFFu;
            mohgeom::t_gifPacketBytes = 0u;
            mohgeom::t_gifByteOffset = 0xFFFFFFFFu;
            mohgeom::t_gifTagIndex = 0u;
            mohgeom::t_gifPrimitiveIndex = 0u;
        }
    ...
    const char *mohGsRegName(uint8_t regAddr)
    {
        switch (regAddr)
        {
        case GS_REG_PRIM:
    ...
    bool mohIsInterestingGsReg(uint8_t regAddr)
    {
        switch (regAddr)
        {
        case GS_REG_PRIM:
                                          ...
                                          bool *outUsedPreferred) const
{
    ...
    {
        const uint64_t tick = ps2_syscalls::GetCurrentVSyncTick();
        static std::atomic<uint32_t> s_mohFrontendPresent{0u};
        if (tick >= 5260u)
        {
            const uint32_t n = s_mohFrontendPresent.fetch_add(1u, std::memory_order_relaxed);
            if (n < 160u)
            {
                uint32_t nonBlackRgb = 0u;
                uint32_t nonZeroAlpha = 0u;
                uint64_t rgbSum = 0u;
                uint8_t rgbPeak = 0u;
                for (uint32_t y = 0; y < m_hostPresentationHeight; ++y)
                {
                    const size_t rowOffset = static_cast<size_t>(y) * kHostFrameWidth * 4u;
                    for (uint32_t x = 0; x < m_hostPresentationWidth; ++x)
                    {
                        const size_t offset = rowOffset + static_cast<size_t>(x) * 4u;
                        const uint8_t r = m_hostPresentationFrame[offset + 0u];
                        const uint8_t g = m_hostPresentationFrame[offset + 1u];
                        const uint8_t b = m_hostPresentationFrame[offset + 2u];
                        const uint8_t a = m_hostPresentationFrame[offset + 3u];
                        nonBlackRgb += (r | g | b) != 0u ? 1u : 0u;
                        nonZeroAlpha += a != 0u ? 1u : 0u;
                        rgbSum += static_cast<uint64_t>(r) + g + b;
                        rgbPeak = std::max(rgbPeak, std::max(r, std::max(g, b)));
                    }
                }
                const auto samplePixel = [&](uint32_t x, uint32_t y) -> uint32_t
                {
                    if (x >= m_hostPresentationWidth || y >= m_hostPresentationHeight)
                        return 0u;
                    const size_t offset = (static_cast<size_t>(y) * kHostFrameWidth + x) * 4u;
                    return static_cast<uint32_t>(m_hostPresentationFrame[offset + 0u]) |
                           (static_cast<uint32_t>(m_hostPresentationFrame[offset + 1u]) << 8u) |
                           (static_cast<uint32_t>(m_hostPresentationFrame[offset + 2u]) << 16u) |
                           (static_cast<uint32_t>(m_hostPresentationFrame[offset + 3u]) << 24u);
                };
                std::cerr << "[MOH:menu-present] n=" << n
                          << " tick=" << tick
                          << " pmode=0x" << std::hex << (m_privRegs ? m_privRegs->pmode : 0u)
                          << " dispfb1=0x" << (m_privRegs ? m_privRegs->dispfb1 : 0u)
                          << " dispfb2=0x" << (m_privRegs ? m_privRegs->dispfb2 : 0u)
                          << " display1=0x" << (m_privRegs ? m_privRegs->display1 : 0u)
                          << " display2=0x" << (m_privRegs ? m_privRegs->display2 : 0u)
                          << " ctx0=(0x" << m_ctx[0].frame.fbp
                          << ",0x" << m_ctx[0].frame.fbw
                          << ",0x" << static_cast<uint32_t>(m_ctx[0].frame.psm) << ")"
                          << " ctx1=(0x" << m_ctx[1].frame.fbp
                          << ",0x" << m_ctx[1].frame.fbw
                          << ",0x" << static_cast<uint32_t>(m_ctx[1].frame.psm) << ")"
                          << " latched=(display=0x" << m_hostPresentationDisplayFbp
                          << ",source=0x" << m_hostPresentationSourceFbp << ")"
                          << std::dec
                          << " size=" << m_hostPresentationWidth << "x" << m_hostPresentationHeight
                          << " rgbNonBlack=" << nonBlackRgb
                          << " alphaNonZero=" << nonZeroAlpha
                          << " rgbSum=" << rgbSum
                          << " rgbPeak=" << static_cast<uint32_t>(rgbPeak)
                          << " samples=(0x" << std::hex << samplePixel(0u, 0u)
                          << ",0x" << samplePixel(320u, 80u)
                          << ",0x" << samplePixel(320u, 224u)
                          << ",0x" << samplePixel(639u, 447u) << ")"
                          << std::dec
                          << std::endl;
            }
        }
...
void GS::processGIFPacket(const uint8_t *data, uint32_t sizeBytes)
{
    ...
    // [MOH diag] reliable (non-gated) count of GIF packets actually processed by the GS.
    static std::atomic<uint32_t> s_mohGifPkt{0};
    const uint32_t mohPacketIndex = s_mohGifPkt.fetch_add(1, std::memory_order_relaxed);
    ScopedMohGifPacket mohPacketScope(mohPacketIndex, sizeBytes);
    {
        if (mohPacketIndex < 64u)
        {
            const uint64_t tagLo0 = loadLE64(data);
            const uint64_t tagHi0 = loadLE64(data + 8);
            const uint32_t nloop = static_cast<uint32_t>(tagLo0 & 0x7FFFu);
            const uint8_t flg = static_cast<uint8_t>((tagLo0 >> 58) & 0x3u);
            uint32_t nreg = static_cast<uint32_t>((tagLo0 >> 60) & 0xFu);
            if (nreg == 0u) nreg = 16u;
            std::cerr << "[MOH:gs-pkt] n=" << mohPacketIndex << " size=" << sizeBytes
                      << " nloop=" << nloop << " flg=" << static_cast<uint32_t>(flg)
                      << " nreg=" << nreg
                      << " regs=0x" << std::hex << tagHi0
                      << " ctx0fbp=0x" << m_ctx[0].frame.fbp
    ...
    {
        mohgeom::t_gifByteOffset = offset;
        ...
        // Which GIFtag modes arrive on which path? flg: 0=PACKED, 1=REGLIST,
        // 2=IMAGE, 3=disabled. An unhandled REGLIST on PATH2 would silently
        // drop the register writes it carries, which would explain PATH2
        // primitives inheriting S, T and Q from PATH1.
        {
            static std::atomic<uint64_t> s_flgCount[4][4] = {};
            const uint32_t pathIdx = mohgeom::t_gifPathId < 4u ? mohgeom::t_gifPathId : 0u;
            const uint64_t c = s_flgCount[pathIdx][flg & 3u].fetch_add(1, std::memory_order_relaxed);
            if (c == 0u || ((c % 20000u) == 19999u))
            {
                std::cerr << "[MOH:giftag-flg-v1] histogram";
                for (uint32_t pth = 0u; pth < 4u; ++pth)
                    for (uint32_t f = 0u; f < 4u; ++f)
                    {
                        const uint64_t v = s_flgCount[pth][f].load(std::memory_order_relaxed);
                        if (v) std::cerr << " p" << pth << "f" << f << "=" << v;
                    }
                std::cerr << std::endl;
            }
        }

        // Register order in PACKED packets decides how Q reaches the vertex: an
        // ST before RGBAQ latches Q for that same vertex, an ST after it makes
        // the vertex consume the previous one's Q. Log the descriptor sequence
        // for the first few textured PACKED packets.
        {
            // Per-path counter: a shared one is exhausted by PATH1 traffic long
            // before a PATH2 packet appears.
            static std::atomic<uint32_t> s_regsSeenP2{0};
            const uint32_t k = (flg == 0u && mohgeom::t_gifPathId == 2u)
                                   ? s_regsSeenP2.fetch_add(1u, std::memory_order_relaxed)
                                   : 0xFFFFFFFFu;
            if (k < 16u)
            {
                std::cerr << "[MOH:giftag-regs-p2-v1] k=" << std::dec << k
                          << " path=" << mohgeom::t_gifPathId
                          << " nloop=" << nloop << " nreg=" << nreg << " order=";
                for (uint32_t i = 0; i < nreg; ++i)
                {
                    static const char *kName[16] = {
                        "PRIM", "RGBAQ", "ST", "UV", "XYZF2", "XYZ2", "TEX0_1",
                        "TEX0_2", "CLAMP_1", "CLAMP_2", "FOG", "?", "XYZF3",
                        "XYZ3", "?", "NOP"};
                    std::cerr << (i ? "," : "") << kName[regs[i] & 0xF];
                }
                std::cerr << std::endl;
            }
        }
        ...
        {
            // Count vertex descriptors decoded against nloop. A shortfall would
            // leave stale vertices in the queue, which is what would make strip
            // triangles fan out from an old position.
            uint32_t vtxDescs = 0u, truncated = 0u;
            for (uint32_t loop = 0; loop < nloop; ++loop)
            {
                for (uint32_t r = 0; r < nreg; ++r)
                {
                    if (offset + 16 > sizeBytes)
                    {
                        ...
                        {
                            static std::atomic<uint64_t> s_trunc{0};
                            const uint64_t c = s_trunc.fetch_add(1, std::memory_order_relaxed);
                            if (c < 6u || (c % 5000u) == 4999u)
                                std::cerr << "[MOH:packet-truncated-v1] n=" << c
                                          << " loop=" << loop << "/" << nloop
                                          << " reg=" << r << "/" << nreg
                                          << " offset=" << offset
                    ...
                    offset += 16;
                    {
                        const uint32_t rd = (uint32_t)((tagHi >> (r * 4)) & 0xFu);
                        if (rd == 0x04u || rd == 0x05u) ++vtxDescs;
                    }
                    writeRegisterPacked(regs[r], lo, hi);
                }
            }
            if (mohgeom::t_gifPathId == 1u && !truncated)
            {
                static std::atomic<uint64_t> s_okCount{0}, s_badCount{0};
                uint32_t expectVtx = 0u;
                for (uint32_t r = 0u; r < nreg; ++r)
                {
                    const uint32_t rd = (uint32_t)((tagHi >> (r * 4)) & 0xFu);
                    if (rd == 0x04u || rd == 0x05u) ++expectVtx;
                }
                expectVtx *= nloop;
                if (vtxDescs == expectVtx) s_okCount.fetch_add(1, std::memory_order_relaxed);
                else s_badCount.fetch_add(1, std::memory_order_relaxed);
                const uint64_t tot = s_okCount.load(std::memory_order_relaxed) +
                                     s_badCount.load(std::memory_order_relaxed);
                if ((tot % 20000u) == 19999u)
                    std::cerr << "[MOH:vtxdesc-check-v1] ok="
                              << s_okCount.load(std::memory_order_relaxed)
                              << " mismatch=" << s_badCount.load(std::memory_order_relaxed)
                              << std::endl;
            }
        }
...
void GS::writeRegisterPacked(uint8_t regDesc, uint64_t lo, uint64_t hi)
{
    ...
    case 0x02:
    {
        ...
        std::memcpy(&m_internalQ, &qBits, 4);
        if (m_internalQ == 0.0f)
            m_internalQ = 1.0f;
        // Raw ST descriptor as it arrives. If the packet itself carries a
        // negative Q then the sign is genuine data; if it never does, the
        // negative Q seen at the primitive level comes from mixing vertices
        // across two fans.
        {
            // Count only PATH2 descriptors: counting every path exhausted the
            // cap on PATH1 traffic before any PATH2 one appeared.
            static std::atomic<uint32_t> s_stSeen{0};
            const uint32_t k = (mohgeom::t_gifPathId == 2u)
                                   ? s_stSeen.fetch_add(1u, std::memory_order_relaxed)
                                   : 0xFFFFFFFFu;
            if (k < 24u)
                std::cerr << "[MOH:raw-st-v1] k=" << std::dec << k
                          << " path=" << mohgeom::t_gifPathId
                          << std::hex << " sBits=" << sBits << " tBits=" << tBits
                          << " qBits=" << qBits << std::dec
                          << " q=" << m_internalQ << std::endl;
        }
        break;
    }
    case 0x03:
    ...
    case 0x04:
    {
        ...
        bool adk = ((hi >> 47) & 1) != 0;
        // VU1 sets the ADC bit (qword bit 111) on every wrapped vertex and on ~95% of all
        // vertices at site 0x1440. If the GS sees a far lower rate, the bit is being lost
        // between the VU1 packet and here, which would explain off-screen geometry being
        // drawn with wrapped coordinates.
        {
            static std::atomic<uint64_t> s_n{0}, s_adc{0};
            const uint64_t c = s_n.fetch_add(1, std::memory_order_relaxed);
            if (adk) s_adc.fetch_add(1, std::memory_order_relaxed);
            if ((c % 20000u) == 19999u)
                std::fprintf(stderr, "[MOH:gs-adc-v1] XYZF2 packed: ADC=%llu/%llu\n",
                             (unsigned long long)s_adc.load(std::memory_order_relaxed),
    ...
    case 0x05:
    {
        ...
        bool adk = ((hi >> 47) & 1) != 0;
        {
            ...
            if (adk) s_adc.fetch_add(1, std::memory_order_relaxed);
            if ((c % 20000u) == 19999u)
                std::fprintf(stderr, "[MOH:gs-adc-v1] XYZ2 packed: ADC=%llu/%llu\n",
                             (unsigned long long)s_adc.load(std::memory_order_relaxed),
                             (unsigned long long)c + 1u);
        }
        PS2_IF_AGRESSIVE_LOGS({
    ...
    case 0x0E:
    {
        ...
        // Which registers does each path actually write through A+D? PATH2 draws
        // exclusively this way, so this histogram says whether the game supplies
        // RGBAQ (and therefore Q) for its primitives or relies on inherited state.
        {
            static std::atomic<uint64_t> s_adReg[4][256] = {};
            const uint32_t pathIdx = mohgeom::t_gifPathId < 4u ? mohgeom::t_gifPathId : 0u;
            const uint64_t c = s_adReg[pathIdx][addr].fetch_add(1, std::memory_order_relaxed);
            if (c == 0u || (c % 50000u) == 49999u)
            {
                std::cerr << "[MOH:ad-reg-hist-v1] path=" << pathIdx << " regs:";
                for (uint32_t r = 0u; r < 256u; ++r)
                {
                    const uint64_t v = s_adReg[pathIdx][r].load(std::memory_order_relaxed);
                    if (v) std::cerr << " 0x" << std::hex << r << "=" << std::dec << v;
                }
                std::cerr << std::endl;
            }
        }
...
void GS::writeRegister(uint8_t regAddr, uint64_t value)
{
    ...
    {
        static std::atomic<uint32_t> s_mohRegTrace{0};
        const uint32_t packetIndex = g_mohCurrentGifPacket;
        if (packetIndex != kNoMohGifPacket &&
            packetIndex >= 7u &&
            packetIndex < 80u &&
            mohIsInterestingGsReg(regAddr))
        {
            const uint32_t n = s_mohRegTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 192u)
            {
                std::cerr << "[MOH:gs-reg] pkt=" << packetIndex
                          << " reg=0x" << std::hex << static_cast<uint32_t>(regAddr)
                          << "(" << mohGsRegName(regAddr) << ")"
                          << " value=0x" << value
    ...
    case GS_REG_PRIM:
    {
        ...
        // Strip-length histogram: how many vertices arrived since the previous
        // PRIM write on this path. A strip running far longer than the meshes it
        // describes is what would bridge unrelated objects and produce the long
        // radiating slivers.
        {
            static thread_local uint32_t s_sinceStrip = 0u;
            static std::atomic<uint64_t> s_len[7] = {};
            const uint32_t v = s_sinceStrip;
            s_sinceStrip = 0u;
            if (mohgeom::t_gifPathId == 1u)
            {
                const uint32_t bkt = v == 0u    ? 0u
                                     : v <= 4u   ? 1u
                                     : v <= 16u  ? 2u
                                     : v <= 64u  ? 3u
                                     : v <= 256u ? 4u
                                     : v <= 1024u ? 5u : 6u;
                const uint64_t c = s_len[bkt].fetch_add(1, std::memory_order_relaxed);
                if ((c % 20000u) == 19999u)
                    std::cerr << "[MOH:strip-len-v1] 0=" << s_len[0].load(std::memory_order_relaxed)
                              << " <=4=" << s_len[1].load(std::memory_order_relaxed)
                              << " <=16=" << s_len[2].load(std::memory_order_relaxed)
                              << " <=64=" << s_len[3].load(std::memory_order_relaxed)
                              << " <=256=" << s_len[4].load(std::memory_order_relaxed)
                              << " <=1024=" << s_len[5].load(std::memory_order_relaxed)
                              << " >1024=" << s_len[6].load(std::memory_order_relaxed)
                              << std::endl;
            }
            mohgeom::g_vtxSinceStrip = &s_sinceStrip;
        }
        m_vtxCount = 0;
        m_vtxIndex = 0;
        // Who sets the primitive type on each path? If PATH2 never writes PRIM,
        // its primitives inherit whatever type PATH1 last used - which would
        // make a full-screen blit render as a triangle fan.
        {
            static std::atomic<uint64_t> s_primByPath[4] = {};
            const uint32_t pathIdx = mohgeom::t_gifPathId < 4u ? mohgeom::t_gifPathId : 0u;
            const uint64_t c = s_primByPath[pathIdx].fetch_add(1, std::memory_order_relaxed);
            if (c == 0u || (c % 20000u) == 19999u)
                std::cerr << "[MOH:prim-writes-v1] p0=" << s_primByPath[0].load(std::memory_order_relaxed)
                          << " p1=" << s_primByPath[1].load(std::memory_order_relaxed)
                          << " p2=" << s_primByPath[2].load(std::memory_order_relaxed)
                          << " p3=" << s_primByPath[3].load(std::memory_order_relaxed)
                          << " lastType=" << static_cast<uint32_t>(m_prim.type)
                          << " onPath=" << pathIdx << std::endl;
        }
        break;
    }
    case GS_REG_RGBAQ:
    {
        ...
        if (m_curQ == 0.0f)
            m_curQ = 1.0f;
        // A+D register write: Q travels in the RGBAQ value itself. This is the
        // route PATH2 uses, so it is where the negative Q must originate.
        {
            static std::atomic<uint32_t> s_adSeen{0};
            const uint32_t k = s_adSeen.fetch_add(1u, std::memory_order_relaxed);
            if (k < 20u && mohgeom::t_gifPathId == 2u)
                std::cerr << "[MOH:ad-rgbaq-v1] k=" << std::dec << k
                          << std::hex << " value=" << value
                          << " qBits=" << qBits << std::dec
                          << " q=" << m_curQ << std::endl;
        }
    ...
    case GS_REG_TEX0_2:
    {
        ...
        if (t.psm == GS_PSM_T8 && ps2_syscalls::GetCurrentVSyncTick() >= 60u)
        {
            static std::atomic<uint32_t> s_mohFrontendTex0Trace{0u};
            const uint32_t n = s_mohFrontendTex0Trace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 96u)
            {
                std::cerr << "[MOH:frontend-tex0]"
                          << " n=" << std::dec << n
                          << " tick=" << ps2_syscalls::GetCurrentVSyncTick()
                          << " ctx=" << ci
                          << " tbp=0x" << std::hex << t.tbp0
                          << " tbw=0x" << static_cast<uint32_t>(t.tbw)
                          << " psm=0x" << static_cast<uint32_t>(t.psm)
                          << " tcc=" << std::dec << static_cast<uint32_t>(t.tcc)
                          << " tfx=" << static_cast<uint32_t>(t.tfx)
                          << " size=2^" << std::dec << static_cast<uint32_t>(t.tw)
                          << "x2^" << static_cast<uint32_t>(t.th)
                          << " cbp=0x" << std::hex << t.cbp
                          << " cpsm=0x" << static_cast<uint32_t>(t.cpsm)
                          << " csm=0x" << static_cast<uint32_t>(t.csm)
                          << " csa=0x" << static_cast<uint32_t>(t.csa)
                          << " cld=0x" << static_cast<uint32_t>(t.cld)
                          << " texclut=(0x" << static_cast<uint32_t>(m_texclut.cbw)
                          << ",0x" << static_cast<uint32_t>(m_texclut.cou)
                          << ",0x" << m_texclut.cov << ")"
                          << std::dec << std::endl;
            }
        }
    ...
    case GS_REG_XYOFFSET_2:
    {
        ...
        const uint16_t oldOfx = m_ctx[ci].xyoffset.ofx;
        const uint16_t oldOfy = m_ctx[ci].xyoffset.ofy;
        m_ctx[ci].xyoffset.ofx = static_cast<uint16_t>(value & 0xFFFF);
        m_ctx[ci].xyoffset.ofy = static_cast<uint16_t>((value >> 32) & 0xFFFF);
        {
            static std::atomic<uint32_t> s_mohXyoffsetTrace{0};
            const uint32_t n = s_mohXyoffsetTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 80u)
            {
                std::cerr << "[MOH:xyoffset] n=" << n
                          << " pkt=" << g_mohCurrentGifPacket
                          << " ctx=" << ci
                          << " raw=0x" << std::hex << value
                          << std::dec
                          << " old=(" << (oldOfx >> 4) << "," << (oldOfy >> 4) << ")"
                          << " new=(" << (m_ctx[ci].xyoffset.ofx >> 4)
                          << "," << (m_ctx[ci].xyoffset.ofy >> 4) << ")"
                          << std::endl;
            }
        }
        break;
    ...
    case GS_REG_SCISSOR_2:
    {
        ...
        // [MOH diag] flag a degenerate scissor (x1<x0 || y1<y0) — it rejects every pixel.
        if (m_ctx[ci].scissor.x1 < m_ctx[ci].scissor.x0 ||
            m_ctx[ci].scissor.y1 < m_ctx[ci].scissor.y0)
        {
            std::cerr << "[MOH:scissor] ci=" << ci
                      << " DEGENERATE raw=0x" << std::hex << static_cast<unsigned long long>(value) << std::dec
    ...
    case GS_REG_TRXDIR:
    {
        ...
        if (m_trxdir == 0u &&
            isMohStartBmpTransfer(m_bitbltbuf.dbp,
                                  m_bitbltbuf.dbw,
                                  m_bitbltbuf.dpsm,
                                  m_trxreg.rrw,
                                  m_trxreg.rrh,
                                  ...
                                  m_trxpos.dsay))
        {
            m_bitbltbuf.dbp = 0x3800u;
            static std::atomic<uint32_t> s_mohStartRedirectTrace{0};
            const uint32_t n = s_mohStartRedirectTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 4u)
            {
                std::cerr << "[MOH:start-bmp-upload]"
                          << " mode=manual-gif"
                          << " dbp=0x3800"
                          << " dbw=0x" << std::hex << static_cast<uint32_t>(m_bitbltbuf.dbw)
                          << " dpsm=0x" << static_cast<uint32_t>(m_bitbltbuf.dpsm)
                          << std::dec
                          << " size=" << m_trxreg.rrw << "x" << m_trxreg.rrh
                          << std::endl;
            }
        }

        {
            static std::atomic<uint32_t> s_mohTrxTrace{0};
            const uint32_t tn = s_mohTrxTrace.fetch_add(1u, std::memory_order_relaxed);
            if (tn < 48u)
            {
                std::cerr << "[MOH:trx] n=" << std::dec << tn
                          << " dir=" << m_trxdir
                          << " dbp=0x" << std::hex << m_bitbltbuf.dbp
                          << " dbw=0x" << static_cast<uint32_t>(m_bitbltbuf.dbw)
                          << " dpsm=0x" << static_cast<uint32_t>(m_bitbltbuf.dpsm)
                          << std::dec << " rrw=" << m_trxreg.rrw << " rrh=" << m_trxreg.rrh
                          << " dsax=" << m_trxpos.dsax << " dsay=" << m_trxpos.dsay
                          << " total_px=" << m_transferState.total_pixels
                          << std::endl;
            }
        }

        if (ps2_syscalls::GetCurrentVSyncTick() >= 5500u &&
            m_bitbltbuf.dbp >= 0x3000u && m_bitbltbuf.dbp < 0x4000u)
        {
            static std::atomic<uint32_t> s_mohFrontendTrxTrace{0u};
            const uint32_t n = s_mohFrontendTrxTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 128u)
            {
                std::cerr << "[MOH:frontend-trx]"
                          << " n=" << std::dec << n
                          << " tick=" << ps2_syscalls::GetCurrentVSyncTick()
                          << " dir=" << m_trxdir
                          << " sbp=0x" << std::hex << m_bitbltbuf.sbp
                          << " sbw=0x" << static_cast<uint32_t>(m_bitbltbuf.sbw)
                          << " spsm=0x" << static_cast<uint32_t>(m_bitbltbuf.spsm)
                          << " dbp=0x" << std::hex << m_bitbltbuf.dbp
                          << " dbw=0x" << static_cast<uint32_t>(m_bitbltbuf.dbw)
                          << " dpsm=0x" << static_cast<uint32_t>(m_bitbltbuf.dpsm)
                          << std::dec
                          << " size=" << m_trxreg.rrw << "x" << m_trxreg.rrh
                          << " src=(" << m_trxpos.ssax << "," << m_trxpos.ssay << ")"
                          << " dst=(" << m_trxpos.dsax << "," << m_trxpos.dsay << ")"
                          << " order=" << static_cast<uint32_t>(m_trxpos.dir)
        ...
        if (m_trxdir == 0u &&
            (m_bitbltbuf.dpsm == 0x13u ||
             (m_trxreg.rrw == 640u && m_trxreg.rrh == 448u) ||
             (m_trxreg.rrw == 16u && m_trxreg.rrh == 448u)))
        {
            static std::atomic<uint32_t> s_mohMovieTrxTrace{0u};
            const uint32_t n = s_mohMovieTrxTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 128u)
            {
                std::cerr << "[MOH:movie-gs-upload]"
                          << " n=" << std::dec << n
                          << " tick=" << ps2_syscalls::GetCurrentVSyncTick()
                          << " dir=" << m_trxdir
                          << " dbp=0x" << std::hex << m_bitbltbuf.dbp
                          ...
                          << " spsm=0x" << static_cast<uint32_t>(m_bitbltbuf.spsm)
                          << " dpsm=0x" << static_cast<uint32_t>(m_bitbltbuf.dpsm)
                          << " pmode=0x" << (m_privRegs ? m_privRegs->pmode : 0u)
                          << " dispfb1=0x" << (m_privRegs ? m_privRegs->dispfb1 : 0u)
                          << " dispfb2=0x" << (m_privRegs ? m_privRegs->dispfb2 : 0u)
                          << " ctx0=(0x" << m_ctx[0].frame.fbp
                          << ",0x" << m_ctx[0].frame.fbw
                          ...
                          << " dsax=" << m_trxpos.dsax
                          << " dsay=" << m_trxpos.dsay
                          << " totalPx=" << m_transferState.total_pixels
                          << std::endl;
            }
        }
    ...
    case 0x59:
        if (m_privRegs)
        {
            ...
            static std::atomic<uint32_t> s_mohPrivTrace{0};
            const uint32_t n = s_mohPrivTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 64u)
                std::cerr << "[MOH:priv-reg] pkt=" << g_mohCurrentGifPacket
                          << " reg=DISPFB1 value=0x" << std::hex << value << std::dec << std::endl;
        }
        break;
    case 0x5a:
        if (m_privRegs)
        {
            ...
            if (n < 64u)
                std::cerr << "[MOH:priv-reg] pkt=" << g_mohCurrentGifPacket
                          << " reg=DISPLAY1 value=0x" << std::hex << value << std::dec << std::endl;
        }
        break;
    case 0x5b:
        if (m_privRegs)
        {
            ...
            if (n < 64u)
                std::cerr << "[MOH:priv-reg] pkt=" << g_mohCurrentGifPacket
                          << " reg=DISPFB2 value=0x" << std::hex << value << std::dec << std::endl;
        }
        break;
    case 0x5c:
        if (m_privRegs)
        {
            ...
            if (n < 64u)
                std::cerr << "[MOH:priv-reg] pkt=" << g_mohCurrentGifPacket
                          << " reg=DISPLAY2 value=0x" << std::hex << value << std::dec << std::endl;
        }
        break;
    case 0x5f:
        if (m_privRegs)
...
void GS::vertexKick(bool drawing)
{
    ++m_vtxCount;
    ++m_vtxIndex;
    if (mohgeom::g_vtxSinceStrip)
        ++(*mohgeom::g_vtxSinceStrip);

    {
        static std::atomic<uint32_t> s_mohKickTrace{0};
        const uint32_t packetIndex = g_mohCurrentGifPacket;
        if (packetIndex != kNoMohGifPacket && packetIndex >= 7u && packetIndex < 80u)
        {
            const uint32_t n = s_mohKickTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 128u)
            {
                const auto &ctx = activeContext();
                const GSVertex &v = m_vtxQueue[(m_vtxCount - 1u) % kMaxVerts];
                std::cerr << "[MOH:gs-kick] pkt=" << packetIndex
                          << " n=" << n
                          << " drawing=" << (drawing ? 1u : 0u)
                          << " prim=" << static_cast<uint32_t>(m_prim.type)
                          << " ctxt=" << static_cast<uint32_t>(m_prim.ctxt)
                          << " vtxCount=" << m_vtxCount
                          << " xy=(" << v.x << "," << v.y << ")"
                          << " uv=(" << (v.u >> 4) << "," << (v.v >> 4) << ")"
                          << " rgba=(" << static_cast<uint32_t>(v.r) << ","
                          << static_cast<uint32_t>(v.g) << ","
                          << static_cast<uint32_t>(v.b) << ","
                          << static_cast<uint32_t>(v.a) << ")"
    ...
    // A no-draw kick (XYZ3/XYZF3) still advances the primitive's vertex window
    // exactly like a drawing kick — the PS2 uses it to restart a strip/fan
    // mid-packet.  The SFN frontend font emits each glyph quad as two XYZ3
    // setup vertices followed by two XYZ2 draw vertices; skipping the window
    // slide below for the no-draw kicks left the following draw kick assembling
    // a triangle from stale vertices of the previous glyph, shearing the atlas
    // sample diagonally across the sheet.  Only the actual rasterization is
    // gated on `drawing`; the vertex-window bookkeeping runs for both.
    if (drawing)
    {
...
void GS::processImageData(const uint8_t *data, uint32_t sizeBytes)
{
    ...
    u32 data_offset = 0;
    uint32_t mohFrontendImageTraceIndex = 0u;
    const uint32_t mohFrontendCopiedBefore = m_transferState.copied_pixels;
    const uint32_t mohFrontendTotalBefore = m_transferState.total_pixels;
    const bool mohTraceFrontendImage =
        ps2_syscalls::GetCurrentVSyncTick() >= 5500u &&
        dbp >= 0x3000u && dbp < 0x4000u;
    if (mohTraceFrontendImage)
    {
        static std::atomic<uint32_t> s_mohFrontendImageTrace{0u};
        mohFrontendImageTraceIndex = s_mohFrontendImageTrace.fetch_add(1u, std::memory_order_relaxed);
        if (mohFrontendImageTraceIndex < 128u)
        {
            ...
            std::memcpy(&firstWord, data, std::min<uint32_t>(sizeBytes, sizeof(firstWord)));
            std::cerr << "[MOH:frontend-image]"
                      << " n=" << std::dec << mohFrontendImageTraceIndex
                      << " tick=" << ps2_syscalls::GetCurrentVSyncTick()
                      << " phase=before"
                      << " dbp=0x" << std::hex << dbp
                      << " dbw=0x" << static_cast<uint32_t>(dbw)
                      << " dpsm=0x" << static_cast<uint32_t>(dpsm)
                      << " bytes=0x" << sizeBytes
                      << " first=0x" << firstWord
                      << " hash=0x" << hash
                      << std::dec
                      << " copied=" << mohFrontendCopiedBefore
                      << "/" << mohFrontendTotalBefore
                      << std::endl;
        }
    }

    if (m_transferState.copied_pixels == 0u &&
        dbp == 0x3800u &&
        dbw == 8u &&
        dpsm == GS_PSM_CT24 &&
        rrw == 512u &&
        ...
        dsay == 0u)
    {
        const std::vector<uint8_t> &pixels = mohStartBmpPixels();
        if (!pixels.empty())
        {
            data = pixels.data();
            sizeBytes = static_cast<uint32_t>(pixels.size());
            static std::atomic<uint32_t> s_mohStartPixelTrace{0};
            const uint32_t n = s_mohStartPixelTrace.fetch_add(1u, std::memory_order_relaxed);
            if (n < 4u)
            {
                std::cerr << "[MOH:start-bmp-pixels]"
    ...
    if (mohTraceFrontendImage && mohFrontendImageTraceIndex < 128u)
    {
        std::cerr << "[MOH:frontend-image]"
                  << " n=" << std::dec << mohFrontendImageTraceIndex
                  << " tick=" << ps2_syscalls::GetCurrentVSyncTick()
                  << " phase=after"
                  << " consumed=0x" << std::hex << data_offset
                  << std::dec
