...
namespace mohgeom
{
    // Defined in ps2_gs_rasterizer.cpp.
    extern std::atomic<uint32_t> g_vif1SegAddr[8];
    extern std::atomic<uint32_t> g_vif1SegQwc[8];
    extern std::atomic<uint32_t> g_vif1SegPos;
    constexpr uint32_t kSegCap = 704u;
    extern uint8_t g_vif1SegData[8][kSegCap];
    extern std::atomic<uint32_t> g_vif1SegLen[8];
}

// Installed by PS2Runtime::syncCoreSubsystems. PS2Memory has no runtime pointer,
// so VIF1 i-bit interrupts (INTC cause 5) are dispatched through this hook,
// invoked by processVIF1Data at the exact i-bit stream position.
std::function<void()> g_vif1InterruptCallback;
...
namespace
{
    ...
    inline uint64_t *gsRegPtr(GSRegisters &gs, uint32_t addr)
    {
        ...
        case 0x00E0:
            return &gs.bgcolor;
        case 0x1000:
            return reinterpret_cast<uint64_t*>(&gs.csr);
        case 0x1010:
            return &gs.imr;
        case 0x1040:
            return &gs.busdir;
...
bool PS2Memory::writeIORegister(uint32_t address, uint32_t value)
{
    ...
    if (address >= 0x10008000 && address < 0x1000F000)
    {
        if ((address & 0xFF) == 0x00 && (value & 0x100))
        {
            ...
            {
                // DMAC addresses do not use the EE CPU's virtual-address encoding.
                // In MADR/TADR (and in a DMAtag's SPR bit), bit 31 selects the
                // 16 KiB scratchpad and the low 14 bits are its byte offset.
                // Keep that interpretation local to the DMAC: 0x80000000 is a
                // perfectly ordinary KSEG0 address when used by the EE CPU.
                auto isDmaScratchpadAddress = [&](uint32_t dmaAddr)
                {
                    return (dmaAddr & 0x80000000u) != 0u || isScratchpad(dmaAddr);
                };
                auto translateDmaAddress = [&](uint32_t dmaAddr)
                {
                    if ((dmaAddr & 0x80000000u) != 0u)
                        return dmaAddr & (PS2_SCRATCHPAD_SIZE - 1u);
                    return translateAddress(dmaAddr);
                };
                auto normalizeDmaAddress = [&](uint32_t dmaAddr)
                {
                    if ((dmaAddr & 0x80000000u) != 0u)
                        return PS2_SCRATCHPAD_BASE + (dmaAddr & (PS2_SCRATCHPAD_SIZE - 1u));
                    return dmaAddr;
                };
                ...
                auto enqueueTransfer = [&](uint32_t srcAddr, uint32_t qwCount)
                {
                    ...
                    // Pending transfers are consumed by the generic CPU-address
                    // translator, so convert the DMAC SPR form back to the CPU
                    // scratchpad window at this boundary.
                    pt.srcAddr = normalizeDmaAddress(srcAddr);
                    pt.qwc = qwCount;
                ...
                const uint64_t mohVsyncTick = ps2_syscalls::GetCurrentVSyncTick();
                static std::atomic<uint32_t> s_mohFrontendDmaTrace{0u};
                uint32_t mohFrontendDmaIndex = 0xFFFFFFFFu;
                bool mohTraceFrontendDma = false;
                if ((channelBase == 0x1000A000u || channelBase == 0x10009000u) &&
                    mohVsyncTick >= 5525u &&
                    !(mode == 1u && (m_ioRegisters[channelBase + 0x30] & 0x80000000u) != 0u))
                {
                    mohFrontendDmaIndex = s_mohFrontendDmaTrace.fetch_add(1u, std::memory_order_relaxed);
                    mohTraceFrontendDma = mohFrontendDmaIndex < 320u;
                    if (mohTraceFrontendDma)
                    {
                        std::cerr << "[MOH:frontend-dma]"
                                  << " n=" << std::dec << mohFrontendDmaIndex
                                  << " tick=" << mohVsyncTick
                                  ...
                                  << " asr0=0x" << m_ioRegisters[channelBase + 0x40]
                                  << " asr1=0x" << m_ioRegisters[channelBase + 0x50]
                                  << std::dec << std::endl;
                    }
                }
                ...
                else if (mode == 1)
                {
                    ...
                    const uint32_t initialTagAddr = tagAddr;
                    ...
                    auto appendData = [&](uint32_t srcAddr, uint32_t qwCount)
                    {
                        ...
                        const bool scratch = isDmaScratchpadAddress(srcAddr);
                        uint32_t src = 0;
                        src = translateDmaAddress(srcAddr);
                        const uint8_t *base2;
                        uint32_t maxSz2;
                        if (scratch)
                        {
                    ...
                    auto appendCompactVif1TagData = [&](uint32_t localTagAddr, uint32_t qwCount)
                    {
                        ...
                        tagPhys = translateDmaAddress(localTagAddr);
                    ...
                    while (tagsProcessed < kMaxChainTags)
                    {
                        ...
                        try
                        {
                            physTag = translateDmaAddress(tagAddr);
                        }
                        catch (...)
                        {
                            break;
                        ...
                        const bool addrInSPR = ((tag >> 63) & 0x1ull) != 0ull;
                        if (addrInSPR)
                            addr = 0x80000000u | (addr & (PS2_SCRATCHPAD_SIZE - 1u));
                        ++tagsProcessed;
                        ...
                        if (mohTraceFrontendDma && tagsProcessed <= 24)
                        {
                            std::cerr << "[MOH:frontend-dmatag]"
                                      << " dma=" << std::dec << mohFrontendDmaIndex
                                      << " tag=" << tagsProcessed
                                      << " at=0x" << std::hex << currentTagAddr
                                      << " raw=0x" << tag
                                      << " id=" << std::dec << id
                                      << " qwc=0x" << std::hex << tagQwc
                                      << " irq=" << std::dec << static_cast<uint32_t>(irq)
                                      << " addr=0x" << std::hex << addr
                                      << " spr=" << std::dec << static_cast<uint32_t>(addrInSPR)
                                      << " data=0x" << std::hex << dataAddr
                                      << " next=0x" << tagAddr
                                      << " end=" << std::dec << static_cast<uint32_t>(endChain)
                                      << std::endl;
                        }

                        // Source-chain DMAtags carry 8 bytes of VIF stream in the
                        // upper half for every tag id (CHCR.TTE semantics), not
                        // just the tag-local ids: MOH's frontend texture packets
                        // end with a REFE whose upper half holds the i-bit NOP
                        // that re-arms the 0x10BFE8 upload pump. The tag words
                        // reach VIF before the tag's payload.
                        const bool vifLocalTag =
                        ...
                        if (hasPayload)
                        {
                            // Guest address of each chain segment. For the VIF1
                            // channel this is where the PATH2 DIRECT display list
                            // actually lives, which is what identifies the EE code
                            // that builds the screen-filling primitives.
                            if (channelBase == 0x10009000u)
                            {
                                {
                                    const uint32_t slot =
                                        mohgeom::g_vif1SegPos.fetch_add(1u, std::memory_order_relaxed) & 7u;
                                    mohgeom::g_vif1SegAddr[slot].store(dataAddr, std::memory_order_relaxed);
                                    mohgeom::g_vif1SegQwc[slot].store(tagQwc, std::memory_order_relaxed);
                                    const uint32_t nBytes =
                                        std::min<uint32_t>(tagQwc * 16u, mohgeom::kSegCap);
                                    for (uint32_t b = 0u; b < nBytes; ++b)
                                        mohgeom::g_vif1SegData[slot][b] =
                                            m_rdram[(dataAddr + b) & 0x01FFFFFFu];
                                    mohgeom::g_vif1SegLen[slot].store(nBytes, std::memory_order_relaxed);
                                }
                                static std::atomic<uint32_t> s_segSeen{0};
                                const uint32_t k =
                                    s_segSeen.fetch_add(1u, std::memory_order_relaxed);
                                // Cap counted per-size: a shared counter gets
                                // exhausted by other segments before the size of
                                // interest ever appears.
                                // Carriers identified by correlation: the 41-qword
                                // segments in the two low display-list buffers
                                // (0x537AE0 / 0x4A75B0 range), not the unrelated
                                // 41-qword segment at 0x183C500 from an earlier
                                // phase. Counter is per-condition.
                                static std::atomic<uint32_t> s_seg41{0};
                                if (tagQwc == 41u && dataAddr < 0x600000u &&
                                    s_seg41.fetch_add(1u, std::memory_order_relaxed) < 2u)
                                {
                                    // Dump the largest segment so its structure
                                    // can be compared with PCSX2 at the same
                                    // guest address.
                                    std::cerr << "[MOH:vif1-seg-dump-v1] addr=0x"
                                              << std::hex << dataAddr << " bytes=";
                                    for (uint32_t b = 0u; b < 656u; ++b)
                                    {
                                        const uint32_t a = (dataAddr + b) & 0x01FFFFFFu;
                                        std::cerr << std::hex << (m_rdram[a] >> 4)
                                                  << (m_rdram[a] & 0xF);
                                    }
                                    std::cerr << std::dec << std::endl;
                                }
                                if (k < 16u)
                                    std::cerr << "[MOH:vif1-seg-v1] k=" << std::dec << k
                                              << " dataAddr=0x" << std::hex << dataAddr
                                              << " qwc=" << std::dec << tagQwc
                    ...
                    if (channelBase == 0x1000A000u && (initialTagAddr & 0x80000000u) != 0u)
                    {
                        static std::atomic<uint32_t> s_sprGifChainTrace{0u};
                        const uint32_t n = s_sprGifChainTrace.fetch_add(1u, std::memory_order_relaxed);
                        if (n < 96u)
                        {
                            std::cerr << "[DMAC:spr-gif-chain] n=" << std::dec << n
                                      << " tadr=0x" << std::hex << initialTagAddr
                                      << " tags=" << std::dec << tagsProcessed
                                      << " bytes=" << chainBuf.size()
                                      << " next=0x" << std::hex << tagAddr
                                      << std::dec << std::endl;
                        }
                    }
...
void PS2Memory::processPendingTransfers()
{
    ...
    // Take ownership before iterating: processVIF1Data dispatches the VIF1
    // i-bit ISR, whose DMA kick re-enters processPendingTransfers. The nested
    // call must only see its own new work, not the list being walked here.
    std::vector<PendingTransfer> vif1Work = std::move(m_pendingVif1Transfers);
    m_pendingVif1Transfers.clear();
    const bool hadVif1 = !vif1Work.empty();
    for (auto &p : vif1Work)
    {
        if (!p.chainData.empty())
        {
            // chainData is a copy, so the pointer seen inside processVIF1Data is
            // not a guest address. srcAddr is the real one, and PATH2 DIRECT
            // packets - the source of the screen-filling mixed-Q primitives -
            // come through here.
            {
                static std::atomic<uint32_t> s_chainSeen{0};
                const uint32_t k = s_chainSeen.fetch_add(1u, std::memory_order_relaxed);
                if (k < 12u)
                    std::cerr << "[MOH:vif1-chain-src-v1] k=" << std::dec << k
                              << " srcAddr=0x" << std::hex << p.srcAddr
                              << " qwc=" << std::dec << p.qwc
                              << " bytes=" << p.chainData.size()
                              << " spr=" << (p.fromScratchpad ? 1 : 0)
                              << std::endl;
            }
...
void PS2Memory::submitGifPacket(GifPathId pathId, const uint8_t *data, uint32_t sizeBytes, bool drainImmediately, bool path2DirectHl)
{
    ...
    if (ps2_syscalls::GetCurrentVSyncTick() >= 5525u)
    {
        static std::atomic<uint32_t> s_mohFrontendGifSubmitTrace{0u};
        const uint32_t n = s_mohFrontendGifSubmitTrace.fetch_add(1u, std::memory_order_relaxed);
        if (n < 320u)
        {
            const uint64_t firstLo = loadScalar<uint64_t>(data, 0u, sizeBytes, "frontend gif trace", 0u);
            const uint64_t firstHi = loadScalar<uint64_t>(data, 8u, sizeBytes, "frontend gif trace", 0u);
            std::cerr << "[MOH:frontend-gif-submit]"
                      << " n=" << std::dec << n
                      ...
                      << " firstLo=0x" << firstLo
                      << " firstHi=0x" << firstHi
                      << " drain=" << std::dec << static_cast<uint32_t>(drainImmediately)
                      << " directHl=" << static_cast<uint32_t>(path2DirectHl)
                      << " masked=" << static_cast<uint32_t>(m_path3Masked)
                      << std::endl;
        }
    }
...
bool PS2Memory::tryProcessNativeGifImageUploadChain(GS &, uint32_t, uint32_t)
{
    return false;
}

bool PS2Memory::tryProcessNativeGifPackedChain(GS &, uint32_t, uint32_t)
{
    return false;
}
