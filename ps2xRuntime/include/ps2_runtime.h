...
public:
    ...
    // [MOH] public so the MOH RPC pass-through (RPC.cpp) can dispatch to the IOP.
    [[nodiscard]] ps2x::iop::RpcResult handleIopRpc(uint8_t *rdram, R5900Context *ctx, ps2x::iop::RpcRequest request);

    // [MOH compat] Under the N=1 fiber executor there is no EE-lock waiter
    // queue; MOH diagnostics that logged the waiter count read 0.
    uint32_t guestExecutionWaiterCountForTesting() const { return 0u; }
    void requestStop();
