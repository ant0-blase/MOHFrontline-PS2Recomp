...
private:
    ...
    float m_curQ = 1.0f;
    // GS internal Q register. In PACKED mode the ST descriptor does not apply Q
    // to the vertex: it latches Q here, and the RGBAQ descriptor is what copies
    // it into the active RGBAQ.Q that vertices consume. Applying Q directly at
    // ST time diverges whenever a packet writes ST without a following RGBAQ.
    float m_internalQ = 1.0f;
